...
#define EXO_PARAM_READWRITE (G_PARAM_READWRITE | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB)
#else
...
static gboolean exo_tree_view_button_press_event(GtkWidget* widget, GdkEventButton* event)
{
    ...
    if (G_LIKELY(event->window == gtk_tree_view_get_bin_window(GTK_TREE_VIEW(tree_view))))
    {
        ...
        tree_view->priv->button_release_activates =
            (tree_view->priv->single_click && event->type == GDK_BUTTON_PRESS && event->button == 1 &&
    ...
    if (event->window == gtk_tree_view_get_bin_window(GTK_TREE_VIEW(tree_view)) && tree_view->priv->single_click)
    {
        ...
        else
        {
                 ...
                 gtk_tree_path_compare(path, tree_view->priv->hover_path) != 0))
            {
                ...
                if (G_LIKELY(path != NULL))
                {
                    /* setup the hand cursor to indicate that the row at the pointer can be activated with a single
                     * click */
