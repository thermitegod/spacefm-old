...
GType exo_icon_view_layout_mode_get_type(void)
{
    ...
    if (type == 0)
    {
        static const GEnumValue values[] = {{EXO_ICON_VIEW_LAYOUT_ROWS, "EXO_ICON_VIEW_LAYOUT_ROWS", "rows"},
...
static void exo_icon_view_get_property(GObject* object, uint prop_id, GValue* value, GParamSpec* pspec);
static void exo_icon_view_set_property(GObject* object, uint prop_id, const GValue* value, GParamSpec* pspec);
...
static void exo_icon_view_forall(GtkContainer* container, gboolean include_internals, GtkCallback callback,
...
static int exo_icon_view_layout_cols(ExoIconView* icon_view, int item_height, int* x, int* maximum_height,
                                      int max_rows);
static int exo_icon_view_layout_rows(ExoIconView* icon_view, int item_width, int* y, int* maximum_width,
                                      int max_cols);
static void exo_icon_view_layout(ExoIconView* icon_view);
static void exo_icon_view_paint_item(ExoIconView* icon_view, ExoIconViewItem* item, GdkRectangle* area,
#if (GTK_MAJOR_VERSION == 3)
                                     cairo_t* cr,
#elif (GTK_MAJOR_VERSION == 2)
                                     GdkDrawable* drawable,
#endif
                                     int x, int y, gboolean draw_focus);
...
static void exo_icon_view_set_cursor_item(ExoIconView* icon_view, ExoIconViewItem* item, int cursor_cell);
...
static gboolean exo_icon_view_item_hit_test(ExoIconView* icon_view, ExoIconViewItem* item, int x, int y, int width,
                                            int height);
...
static void exo_icon_view_calculate_item_size2(ExoIconView* icon_view, ExoIconViewItem* item, int* max_width,
                                               int* max_height);
...
static void exo_icon_view_add_move_binding(GtkBindingSet* binding_set, uint keyval, uint modmask,
                                           GtkMovementStep step, int count);
static gboolean exo_icon_view_real_move_cursor(ExoIconView* icon_view, GtkMovementStep step, int count);
...
static gboolean exo_icon_view_select_all_between(ExoIconView* icon_view, ExoIconViewItem* anchor,
                                                 ExoIconViewItem* cursor);
static ExoIconViewItem* exo_icon_view_get_item_at_coords(const ExoIconView* icon_view, int x, int y,
                                                         gboolean only_in_cell, ExoIconViewCellInfo** cell_at_pos);
static void exo_icon_view_get_cell_area(ExoIconView* icon_view, ExoIconViewItem* item, ExoIconViewCellInfo* cell_info,
                                        GdkRectangle* cell_area);
static ExoIconViewCellInfo* exo_icon_view_get_cell_info(ExoIconView* icon_view, GtkCellRenderer* renderer);
...
static void exo_icon_view_cell_layout_add_attribute(GtkCellLayout* layout, GtkCellRenderer* renderer,
...
static void exo_icon_view_cell_layout_set_cell_data_func(GtkCellLayout* layout, GtkCellRenderer* cell,
                                                         GtkCellLayoutDataFunc func, gpointer func_data,
                                                         GDestroyNotify destroy);
static void exo_icon_view_cell_layout_reorder(GtkCellLayout* layout, GtkCellRenderer* cell, int position);
...
static void exo_icon_view_put(ExoIconView* icon_view, GtkWidget* widget, ExoIconViewItem* item, int cell);
static void exo_icon_view_remove_widget(GtkCellEditable* editable, ExoIconView* icon_view);
static void exo_icon_view_start_editing(ExoIconView* icon_view, ExoIconViewItem* item, ExoIconViewCellInfo* cell_info,
                                        GdkEvent* event);
...
static void exo_icon_view_drag_data_get(GtkWidget* widget, GdkDragContext* context, GtkSelectionData* selection_data,
                                        uint info, uint drag_time);
static void exo_icon_view_drag_data_delete(GtkWidget* widget, GdkDragContext* context);

/* Target side drag signals */
static void exo_icon_view_drag_leave(GtkWidget* widget, GdkDragContext* context, uint drag_time);
static gboolean exo_icon_view_drag_motion(GtkWidget* widget, GdkDragContext* context, int x, int y, uint drag_time);
static gboolean exo_icon_view_drag_drop(GtkWidget* widget, GdkDragContext* context, int x, int y, uint drag_time);
static void exo_icon_view_drag_data_received(GtkWidget* widget, GdkDragContext* context, int x, int y,
                                             GtkSelectionData* selection_data, uint info, uint drag_time);
...
static gboolean exo_icon_view_search_iter(ExoIconView* icon_view, GtkTreeModel* model, GtkTreeIter* iter,
                                          const char* text, int* count, int n);
...
static gboolean exo_icon_view_search_equal_func(GtkTreeModel* model, int column, const char* key, GtkTreeIter* iter,
                                                gpointer user_data);
static void exo_icon_view_search_position_func(ExoIconView* icon_view, GtkWidget* search_dialog, gpointer user_data);
static gboolean exo_icon_view_search_button_press_event(GtkWidget* widget, GdkEventButton* event,
                                                        ExoIconView* icon_view);
...
struct _ExoIconViewItem
{
    ...
    uint row: ((sizeof(guint) / 2) * 8) - 1;
    uint col: ((sizeof(guint) / 2) * 8) - 1;
    uint selected : 1;
...
struct _ExoIconViewPrivate
{
    ...
    uint scroll_to_use_align : 1;

    /* misc flags */
    uint source_set : 1;
    uint dest_set : 1;
    uint reorderable : 1;
    uint empty_view_drop : 1;

    uint ctrl_pressed : 1;
    uint shift_pressed : 1;
    ...
    uint single_click_timeout_id;
    uint single_click_timeout_state;

    /* Interactive search support */
    uint enable_search : 1;
    uint search_imcontext_changed : 1;
    int search_column;
    int search_selected_iter;
    uint search_timeout_id;
...
static uint icon_view_signals[LAST_SIGNAL];
...
#if (GTK_MAJOR_VERSION == 3)
static void exo_icon_view_get_preferred_width(GtkWidget* widget, int* minimal_width, int* natural_width)
{
...
static void exo_icon_view_get_preferred_height(GtkWidget* widget, int* minimal_height, int* natural_height)
{
    GtkRequisition requisition;
...
static void exo_icon_view_class_init(ExoIconViewClass* klass)
{
    ...
    g_object_class_install_property(
        gobject_class,
        PROP_REORDERABLE,
        g_param_spec_boolean("reorderable", "Reorderable", "View is reorderable", FALSE, EXO_PARAM_READWRITE));
    ...
    //  gtk_binding_entry_add_signal (gtkbinding_set, GDK_f, GDK_CONTROL_MASK, "start-interactive-search", 0);   //MOD
    //  stole ctrl-f for create new folder
...
#define GDK_WA_COLORMAP 0
    ...
    attributes.event_mask = GDK_EXPOSURE_MASK | GDK_SCROLL_MASK | GDK_POINTER_MOTION_MASK | GDK_LEAVE_NOTIFY_MASK |
...
static void exo_icon_view_allocate_children(ExoIconView* icon_view)
{
    ...
    for (lp = priv->children; lp != NULL; lp = lp->next)
    {
        ...
        allocation.height =
            MIN(priv->height - allocation.y, allocation.height + 2 * (focus_line_width + focus_padding));
...
static void exo_icon_view_size_allocate(GtkWidget* widget, GtkAllocation* allocation)
{
    ...
    if (gtk_adjustment_get_value(hadjustment) >
        ...
        gtk_adjustment_set_value(
            hadjustment,
            MAX(0, gtk_adjustment_get_upper(hadjustment) - gtk_adjustment_get_page_size(hadjustment)));
    ...
    if (gtk_adjustment_get_value(vadjustment) >
        ...
        gtk_adjustment_set_value(
            vadjustment,
            MAX(0, gtk_adjustment_get_upper(vadjustment) - gtk_adjustment_get_page_size(vadjustment)));
...
#endif
    event_area_last = (priv->layout_mode == EXO_ICON_VIEW_LAYOUT_ROWS) ? event_area.y + event_area.height
...
static gboolean rubberband_scroll_timeout(gpointer user_data)
{
    ...
    adjustment = (icon_view->priv->layout_mode == EXO_ICON_VIEW_LAYOUT_ROWS) ? icon_view->priv->vadjustment
...
static gboolean exo_icon_view_motion_notify_event(GtkWidget* widget, GdkEventMotion* event)
{
    ...
    if (icon_view->priv->doing_rubberband)
    {
        ...
        {
            abso = event->y - icon_view->priv->height * (gtk_adjustment_get_value(icon_view->priv->vadjustment) /
        ...
        {
            abso = event->x - icon_view->priv->width * (gtk_adjustment_get_value(icon_view->priv->hadjustment) /
...
static void exo_icon_view_item_activate_cell(ExoIconView* icon_view, ExoIconViewItem* item, ExoIconViewCellInfo* info,
                                             GdkEvent* event)
{
...
static void exo_icon_view_start_editing(ExoIconView* icon_view, ExoIconViewItem* item, ExoIconViewCellInfo* info,
                                        GdkEvent* event)
{
    GtkCellRendererMode mode;
    GtkCellEditable* editable;
...
static gboolean exo_icon_view_button_press_event(GtkWidget* widget, GdkEventButton* event)
{
    ...
    if (event->button == 1 && event->type == GDK_BUTTON_PRESS)
    {
        ...
        if (item != NULL)
        {
            ...
            else if (icon_view->priv->selection_mode == GTK_SELECTION_MULTIPLE && (event->state & GDK_SHIFT_MASK))
            {
        ...
        else
        {
            ...
            if (icon_view->priv->selection_mode != GTK_SELECTION_BROWSE && !(event->state & GDK_CONTROL_MASK))
            {
...
static gboolean exo_icon_view_key_press_event(GtkWidget* widget, GdkEventKey* event)
{
    ...
    gtk_window_move(GTK_WINDOW(icon_view->priv->search_window), workarea.width + 1, workarea.height + 1);
#elif (GTK_MAJOR_VERSION == 2)
...
static GList* exo_icon_view_layout_single_row(ExoIconView* icon_view, GList* first_item, int item_width, int row,
                                              int* y, int* maximum_width, int max_cols)
{
...
static GList* exo_icon_view_layout_single_col(ExoIconView* icon_view, GList* first_item, int item_height, int col,
                                              int* x, int* maximum_height, int max_rows)
{
    ExoIconViewPrivate* priv = icon_view->priv;
    ExoIconViewItem* item;
    GList* items = first_item;
                                      ...
                                      int max_cols)
{
    ...
    *y = icon_view->priv->margin;
    ...
    do
    {
        icons = exo_icon_view_layout_single_row(icon_view, icons, item_width, row, y, maximum_width, max_cols);
...
static void exo_icon_view_get_cell_area(ExoIconView* icon_view, ExoIconViewItem* item, ExoIconViewCellInfo* info,
                                        GdkRectangle* cell_area)
{
    ...
    else
    {
        ...
        cell_area->height =
            item->box[info->position].height + item->before[info->position] + item->after[info->position];
    }
}
                                     ...
                                     int x, int y, gboolean draw_focus)
{
    ...
    if (G_UNLIKELY(
            /*EXO_ICON_VIEW_FLAG_SET (icon_view, EXO_ICON_VIEW_DRAW_KEYFOCUS) &&*/ icon_view->priv->cursor_item ==
            item))
...
static void exo_icon_view_queue_layout(ExoIconView* icon_view)
{
    ...
    // icon_view->priv->layout_idle_id = gdk_threads_add_idle_full(G_PRIORITY_DEFAULT_IDLE, layout_callback, icon_view,
    // layout_destroy);
}

static void exo_icon_view_set_cursor_item(ExoIconView* icon_view, ExoIconViewItem* item, int cursor_cell)
{
    if (icon_view->priv->cursor_item == item && (cursor_cell < 0 || cursor_cell == icon_view->priv->cursor_cell))
        return;
...
static void exo_icon_view_row_changed(GtkTreeModel* model, GtkTreePath* path, GtkTreeIter* iter, ExoIconView* icon_view)
{
    ExoIconViewItem* item;
...
static void exo_icon_view_row_deleted(GtkTreeModel* model, GtkTreePath* path, ExoIconView* icon_view)
{
    ...
    if (G_UNLIKELY(item == icon_view->priv->anchor_item))
        icon_view->priv->anchor_item =
            (list->next != NULL) ? list->next->data : ((list->prev != NULL) ? list->prev->data : NULL);
...
static void exo_icon_view_rows_reordered(GtkTreeModel* model, GtkTreePath* parent, GtkTreeIter* iter, int* new_order,
                                         ExoIconView* icon_view)
{
    GList** list_array;
    GList* list;
...
static gboolean exo_icon_view_real_move_cursor(ExoIconView* icon_view, GtkMovementStep step, int count)
{
    ...
    _exo_return_val_if_fail(step == GTK_MOVEMENT_LOGICAL_POSITIONS || step == GTK_MOVEMENT_VISUAL_POSITIONS ||
...
static int find_cell(ExoIconView* icon_view, ExoIconViewItem* item, int cell, GtkOrientation orientation, int step,
                      int* count)
{
    int n_focusable;
    int* focusable;
...
static ExoIconViewItem* find_item_page_up_down(ExoIconView* icon_view, ExoIconViewItem* current, int count)
{
...
static void exo_icon_view_move_cursor_up_down(ExoIconView* icon_view, int count)
{
    ...
    if (icon_view->priv->ctrl_pressed || !icon_view->priv->shift_pressed || !icon_view->priv->anchor_item ||
 ...
 */
static void exo_icon_view_get_item_needed_size(ExoIconView* icon_view, ExoIconViewItem* item, int* width, int* height)
{
    GList* lp;
    ExoIconViewCellInfo* info;
    ...
    *height = 0;
    ...
    for (lp = icon_view->priv->cell_list; lp != NULL; lp = lp->next)
    {
        ...
        if (icon_view->priv->orientation == GTK_ORIENTATION_HORIZONTAL)
        {
            *width += item->box[info->position].width + (info->position > 0 ? icon_view->priv->spacing : 0);
            *height = MAX(*height, item->box[info->position].height);
        }
        else
        {
            *width = MAX(*width, item->box[info->position].width);
            *height += item->box[info->position].height + (info->position > 0 ? icon_view->priv->spacing : 0);
        }
    }
}
...
static void exo_icon_view_scroll_to_item(ExoIconView* icon_view, ExoIconViewItem* item)
{
    ...
    if (y + item->area.y - focus_width < 0)
    {
        gtk_adjustment_set_value(icon_view->priv->vadjustment,
                                 gtk_adjustment_get_value(icon_view->priv->vadjustment) + y + item->area.y -
                                     focus_width);
    }
    else if (y + item->area.y + item_height + focus_width > allocation.height && y + item->area.y - focus_width > 0)
    {
        gtk_adjustment_set_value(
            icon_view->priv->vadjustment,
            gtk_adjustment_get_value(icon_view->priv->vadjustment) +
                MIN(y + item->area.y - focus_width, y + item->area.y + item_height + focus_width - allocation.height));
    }

    if (x + item->area.x - focus_width < 0)
    {
        gtk_adjustment_set_value(icon_view->priv->hadjustment,
                                 gtk_adjustment_get_value(icon_view->priv->hadjustment) + x + item->area.x -
                                     focus_width);
    }
    else if (x + item->area.x + item_width + focus_width > allocation.width && x + item->area.x - focus_width > 0)
    {
        gtk_adjustment_set_value(
            icon_view->priv->hadjustment,
            gtk_adjustment_get_value(icon_view->priv->hadjustment) +
                MIN(x + item->area.x - focus_width, x + item->area.x + item_width + focus_width - allocation.width));
    }
 ...
 **/
void exo_icon_view_widget_to_icon_coords(const ExoIconView* icon_view, int wx, int wy, int* ix, int* iy)
{
 ...
 **/
void exo_icon_view_icon_to_widget_coords(const ExoIconView* icon_view, int ix, int iy, int* wx, int* wy)
{
    g_return_if_fail(EXO_IS_ICON_VIEW(icon_view));
 ...
 * Since: 0.3.1
 **/
gboolean exo_icon_view_get_item_at_pos(const ExoIconView* icon_view, int x, int y, GtkTreePath** path,
                                       GtkCellRenderer** cell)
{
    ExoIconViewCellInfo* info = NULL;
    ExoIconViewItem* item;
...
gboolean exo_icon_view_get_visible_range(const ExoIconView* icon_view, GtkTreePath** start_path, GtkTreePath** end_path)
{
    ...
    for (i = 0, lp = priv->items; lp != NULL; ++i, lp = lp->next)
    {
        ...
        if ((item->area.x + item->area.width >= (gint)gtk_adjustment_get_value(priv->hadjustment)) &&
            ...
            (item->area.x <=
             (gint)(gtk_adjustment_get_value(priv->hadjustment) + gtk_adjustment_get_page_size(priv->hadjustment))) &&
            (item->area.y <=
             (gint)(gtk_adjustment_get_value(priv->vadjustment) + gtk_adjustment_get_page_size(priv->vadjustment))))
        {
            if (start_index == -1)
                start_index = i;
            end_index = i;
 ...
 * Since: 0.3.1
 **/
void exo_icon_view_scroll_to_path(ExoIconView* icon_view, GtkTreePath* path, gboolean use_align, float row_align,
                                  float col_align)
{
    ExoIconViewItem* item;
    GtkAllocation allocation;
...
static void set_dest_row(GdkDragContext* context, GtkTreeModel* model, GtkTreePath* dest_row, gboolean empty_view_drop,
                         gboolean drop_append_mode)
{
    DestRow* dr;
...
static gboolean check_model_dnd(GtkTreeModel* model, GType required_iface, const char* _signal)
{
...
static gboolean set_destination(ExoIconView* icon_view, GdkDragContext* context, int x, int y,
...
static GtkTreePath* get_logical_destination(ExoIconView* icon_view, gboolean* drop_append_mode)
{
                                             ...
                                             GtkSelectionData* selection_data, uint info, uint drag_time)
{
    ...
    gtk_drag_finish(context, accepted, (gdk_drag_context_get_selected_action(context) == GDK_ACTION_MOVE), drag_time);
 ...
 * Since: 0.3.1
 **/
void exo_icon_view_enable_model_drag_source(ExoIconView* icon_view, GdkModifierType start_button_mask,
                                            const GtkTargetEntry* targets, int n_targets, GdkDragAction actions)
{
    g_return_if_fail(EXO_IS_ICON_VIEW(icon_view));
 ...
 * Since: 0.3.1
 **/
void exo_icon_view_enable_model_drag_dest(ExoIconView* icon_view, const GtkTargetEntry* targets, int n_targets,
 ...
 * Since: 0.3.1
 **/
gboolean exo_icon_view_get_dest_item_at_pos(ExoIconView* icon_view, int drag_x, int drag_y, GtkTreePath** path,
                                            ExoIconViewDropPosition* pos)
{
    ExoIconViewItem* item;
...
static gboolean exo_icon_view_single_click_timeout(gpointer user_data)
{
    ...
    if (gtk_widget_has_focus(GTK_WIDGET(icon_view)) && icon_view->priv->single_click &&
        icon_view->priv->prelit_item != NULL)
    {
 ...
 * Since: 0.3.1.3
 **/
void exo_icon_view_set_search_equal_func(ExoIconView* icon_view, ExoIconViewSearchEqualFunc search_equal_func,
                                         gpointer search_equal_data, GDestroyNotify search_equal_destroy)
{
    g_return_if_fail(EXO_IS_ICON_VIEW(icon_view));
    g_return_if_fail(search_equal_func != NULL || (search_equal_data == NULL && search_equal_destroy == NULL));

    /* destroy the previous data (if any) */
    if (G_UNLIKELY(icon_view->priv->search_equal_destroy != NULL && icon_view->priv->search_equal_data != NULL)) // sfm
 ...
 * Since: 0.3.1.3
 **/
void exo_icon_view_set_search_position_func(ExoIconView* icon_view, ExoIconViewSearchPositionFunc search_position_func,
                                            gpointer search_position_data, GDestroyNotify search_position_destroy)
{
    g_return_if_fail(EXO_IS_ICON_VIEW(icon_view));
...
static gboolean exo_icon_view_search_start(ExoIconView* icon_view, gboolean keybinding)
{
    ...
    {
        icon_view->priv->search_entry_changed_id = g_signal_connect(G_OBJECT(icon_view->priv->search_entry),
                                                                    "changed",
