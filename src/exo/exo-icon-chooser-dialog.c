...
static void exo_icon_chooser_dialog_entry_changed(GtkWidget* entry, ExoIconChooserDialog* icon_chooser_dialog);
...
static void exo_icon_chooser_dialog_set_model(ExoIconChooserDialog* dialog)
{
    ...
    if (filter == NULL || GTK_TREE_MODEL(model) != gtk_tree_model_filter_get_model(GTK_TREE_MODEL_FILTER(filter)))
    {
    ...
    else
    {
        ...
        if (_exo_icon_chooser_model_get_iter_for_icon_name(EXO_ICON_CHOOSER_MODEL(model), &model_iter, icon))
        {
            ...
            if (G_LIKELY(model_path != NULL))
            {
                ...
                filter_path =
                    gtk_tree_model_filter_convert_child_path_to_path(GTK_TREE_MODEL_FILTER(filter), model_path);
