...
static void fm_main_window_get_property(GObject* obj, uint prop_id, GValue* value, GParamSpec* pspec);
static void fm_main_window_set_property(GObject* obj, uint prop_id, const GValue* value, GParamSpec* pspec);
...
static void on_folder_notebook_switch_pape(GtkNotebook* notebook, GtkWidget* page, uint page_num, gpointer user_data);
...
static void on_file_browser_open_item(PtkFileBrowser* file_browser, const char* path, PtkOpenAction action,
                                      FMMainWindow* main_window);
...
static gboolean on_tab_drag_motion(GtkWidget* widget, GdkDragContext* drag_context, int x, int y, uint time,
...
GtkWidget* create_plugins_menu(FMMainWindow* main_window)
{
    ...
    XSet* set = xset_set_cb("plug_cfile", on_plugin_install, main_window);
    ...
    item = xset_add_menuitem(file_browser, plug_menu, accel_group, set);
    if (item)
    {
...
GtkWidget* create_devices_menu(FMMainWindow* main_window)
{
    ...
    XSet* set = xset_set_cb("main_dev", on_devices_show, main_window);
...
void main_window_rubberband_all()
{
    ...
    for (l = all_windows; l; l = l->next)
    {
        ...
        for (p = 1; p < 5; p++)
        {
            ...
            {
                PtkFileBrowser* a_browser = PTK_FILE_BROWSER(gtk_notebook_get_nth_page(GTK_NOTEBOOK(notebook), i));
...
void main_update_fonts(GtkWidget* widget, PtkFileBrowser* file_browser)
{
    ...
    {
        FMMainWindow* main_window = (FMMainWindow*)l->data;
        XSet* set = xset_get_panel(p, "icon_status");
        ...
        for (i = 0; i < num_pages; i++)
        {
            ...
            PtkFileBrowser* a_browser = PTK_FILE_BROWSER(gtk_notebook_get_nth_page(GTK_NOTEBOOK(main_window->panel[p - 1]), i));
            // file list / tree
            ...
            GtkWidget* tab_label = fm_main_window_create_tab_label(main_window, a_browser);
...
void main_design_mode(GtkMenuItem* menuitem, FMMainWindow* main_window)
{
    xset_msg_dialog(
        ...
        NULL,
        0,
        _("Design Mode allows you to change the name, shortcut key and icon of menu, toolbar and bookmark items, show "
          "help for an item, and add your own custom commands and applications.\n\nTo open the Design Menu, simply "
          "right-click on a menu item, bookmark, or toolbar item.  To open the Design Menu for a submenu, first close "
...
void main_window_rebuild_all_toolbars(PtkFileBrowser* file_browser)
{
    ...
    for (l = all_windows; l; l = l->next)
    {
        ...
        for (p = 1; p < 5; p++)
        {
            ...
            {
                PtkFileBrowser* a_browser = PTK_FILE_BROWSER(gtk_notebook_get_nth_page(GTK_NOTEBOOK(notebook), cur_tabx));
...
void main_window_toggle_thumbnails_all_windows()
{
    ...
    for (l = all_windows; l; l = l->next)
    {
        ...
        for (p = 1; p < 5; p++)
        {
            GtkWidget* notebook = GTK_NOTEBOOK(a_window->panel[p - 1]);
            int n = gtk_notebook_get_n_pages(notebook);
            int i;
            for (i = 0; i < n; ++i)
            {
                PtkFileBrowser* file_browser = PTK_FILE_BROWSER(gtk_notebook_get_nth_page(notebook, i));
    ...
    if (main_window->panel_change)
    {
        for (p = 1; p < 5; p++)
        {
            if (gtk_widget_get_visible(GTK_WIDGET(main_window->panel[p - 1])))
            {
                ...
                {
                    file_browser =
                        PTK_FILE_BROWSER(gtk_notebook_get_nth_page(GTK_NOTEBOOK(main_window->panel[p - 1]), cur_tabx));
                    if (file_browser)
                    {
        ...
        if (show[p])
        {
            ...
            if (!gtk_notebook_get_n_pages(GTK_NOTEBOOK(main_window->panel[p - 1])))
            {
                ...
                if ((set->s && app_settings.load_saved_tabs) || set->ob1)
                {
                    ...
                    char* tabs_add = g_strdup_printf("%s%s%s",
                    ...
                    if (set->x && !set->ob1)
                    {
                        ...
                        cur_tabx = atoi(set->x);
                        if (cur_tabx >= 0 &&
                            cur_tabx < gtk_notebook_get_n_pages(GTK_NOTEBOOK(main_window->panel[p - 1])))
                        {
    ...
    if (!xset_get_b_panel(main_window->curpanel, "show"))
    {
        ...
        while (p != main_window->curpanel)
        {
            if (xset_get_b_panel(p, "show"))
            {
                ...
                file_browser =
                    (PtkFileBrowser*)gtk_notebook_get_nth_page(GTK_NOTEBOOK(main_window->notebook), cur_tabx);
                gtk_widget_grab_focus(file_browser->folder_view);
                break;
            }
            p++;
...
void rebuild_menus(FMMainWindow* main_window)
{
    ...
    menu_elements = g_strdup_printf("main_save_session main_search sep_f1 main_terminal main_root_terminal "
    ...
    char* menu_elements2 = g_strdup_printf(
        "sep_v1 main_tasks main_auto sep_v2 main_title main_icon main_full sep_v3 main_design_mode main_prefs");
    ...
    xset_add_menuitem(file_browser, newmenu, accel_group, ptk_bookmark_view_get_first_bookmark(NULL));
    gtk_widget_show_all(GTK_WIDGET(newmenu));
    g_signal_connect(newmenu, "key-press-event", G_CALLBACK(xset_menu_keypress), NULL);
...
void fm_main_window_init(FMMainWindow* main_window)
{
    ...
    {
        theme_change_notify =
            g_signal_connect(gtk_icon_theme_get_default(), "changed", G_CALLBACK(update_window_icons), NULL);
    }
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        const char* icon_name = g_strdup_printf(_("Show Panel %d"), i + 1);
        gtk_tool_button_set_label(GTK_TOOL_BUTTON(main_window->panel_btn[i]), icon_name);
        // g_free( icon_name );
        free((char*)icon_name);

        XSet* set = xset_get_panel(i + 1, "icon_status");
        if (set->icon && set->icon[0] != '\0')
            icon_name = set->icon;
        else
            icon_name = "gtk-yes";
...
void fm_main_window_get_property(GObject* obj, uint prop_id, GValue* value, GParamSpec* pspec)
{
}

void fm_main_window_set_property(GObject* obj, uint prop_id, const GValue* value, GParamSpec* pspec)
{
}
...
void fm_main_window_store_positions(FMMainWindow* main_window)
{
    ...
    if (!main_window->fullscreen)
    {
        ...
        {
            char* posa;
            int pos = gtk_paned_get_position(GTK_PANED(main_window->hpane_top));
            if (pos)
            {
        ...
        for (p = 1; p < 5; p++)
        {
            int page_x = gtk_notebook_get_current_page(GTK_NOTEBOOK(main_window->panel[p - 1]));
            if (page_x != -1)
            {
                PtkFileBrowser* a_browser =
                    PTK_FILE_BROWSER(gtk_notebook_get_nth_page(GTK_NOTEBOOK(main_window->panel[p - 1]), page_x));
...
static gboolean fm_main_window_window_state_event(GtkWidget* widget, GdkEventWindowState* event)
{
    ...
    main_window->maximized = app_settings.maximized = ((event->new_window_state & GDK_WINDOW_STATE_MAXIMIZED) != 0);
    if (!main_window->maximized)
    {
...
char* main_window_get_tab_cwd(PtkFileBrowser* file_browser, int tab_num)
{
    ...
    {
        return g_strdup(
            ptk_file_browser_get_cwd(PTK_FILE_BROWSER(gtk_notebook_get_nth_page(GTK_NOTEBOOK(notebook), page_x))));
    }
    else
        return NULL;
}
...
void on_close_notebook_page(GtkButton* btn, PtkFileBrowser* file_browser)
{
    ...
    if (evt_tab_close->s || evt_tab_close->ob2_data)
        main_window_event(main_window,
                          ...
                          gtk_notebook_page_num(GTK_NOTEBOOK(main_window->notebook), GTK_WIDGET(file_browser)) + 1,
                          NULL,
                          0,
                          0,
                          0,
                          ...
                          PtkFileBrowser* file_browser) // MOD added
{
    ...
    if ((evt_win_click->s || evt_win_click->ob2_data) && main_window_event(file_browser->main_window,
                                                                           evt_win_click,
                                                                           "evt_win_click",
                                                                           0,
                                                                           0,
...
{
    GtkWidget* label = gtk_notebook_get_tab_label(GTK_NOTEBOOK(main_window->notebook), GTK_WIDGET(file_browser));
    if (label)
    {
...
void fm_main_window_add_new_tab(FMMainWindow* main_window, const char* folder_path)
{
    ...
    PtkFileBrowser* file_browser =
        (PtkFileBrowser*)ptk_file_browser_new(main_window->curpanel, notebook, main_window->task_view, main_window);
    if (!file_browser)
        return;
...
void set_panel_focus(FMMainWindow* main_window, PtkFileBrowser* file_browser)
{
    ...
    for (p = 1; p < 5; p++)
    {
        GtkWidget* notebook = mw->panel[p - 1];
        int pages = gtk_notebook_get_n_pages(GTK_NOTEBOOK(notebook));
        int cur_tabx;
        for (cur_tabx = 0; cur_tabx < pages; cur_tabx++)
        {
...
void set_window_title(FMMainWindow* main_window, PtkFileBrowser* file_browser)
{
    ...
    if (strstr(fmt, "%t") || strstr(fmt, "%T") || strstr(fmt, "%p") || strstr(fmt, "%P"))
    {
        ...
        char* tab_count = g_strdup_printf("%d", itab_count);
        char* tab_num = g_strdup_printf("%d", itab_num);
        char* panel_count = g_strdup_printf("%d", ipanel_count);
        char* panel_num = g_strdup_printf("%d", main_window->curpanel);
...
void on_folder_notebook_switch_pape(GtkNotebook* notebook, GtkWidget* page, uint page_num, gpointer user_data)
{
    FMMainWindow* main_window = FM_MAIN_WINDOW(user_data);
    ...
    PtkFileBrowser* file_browser = PTK_FILE_BROWSER(gtk_notebook_get_nth_page(notebook, page_num));
...
void on_file_browser_open_item(PtkFileBrowser* file_browser, const char* path, PtkOpenAction action,
                               FMMainWindow* main_window)
{
    if (G_LIKELY(path))
    {
...
void fm_main_window_update_status_bar(FMMainWindow* main_window, PtkFileBrowser* file_browser)
{
    ...
    guint64 total_size;
    int num_sel = ptk_file_browser_get_n_sel(file_browser, &total_size);
    int num_vis = ptk_file_browser_get_n_visible_files(file_browser);
...
gboolean on_tab_drag_motion(GtkWidget* widget, GdkDragContext* drag_context, int x, int y, uint time,
                            PtkFileBrowser* file_browser)
{
...
static gboolean on_main_window_keypress(FMMainWindow* main_window, GdkEventKey* event, XSet* known_set)
{
    ...
    if ((event->keyval == GDK_KEY_Home && (keymod == 0 || keymod == GDK_SHIFT_MASK)) ||
        ...
        (event->keyval == GDK_KEY_Delete && keymod == 0) || (event->keyval == GDK_KEY_Tab && keymod == 0) ||
    ...
    for (l = xsets; l; l = l->next)
    {
        ...
        // nonlatin key match is for nonlatin keycodes set prior to 1.0.3
        if ((((XSet*)l->data)->key == event->keyval || (nonlatin_key && ((XSet*)l->data)->key == nonlatin_key)) &&
            ...
            else if (g_str_has_prefix(set->name, "task_"))
            {
                ...
                if (strstr(xname, "_manager"))
                    ...
                    on_task_column_selected(NULL, browser->task_view);
                         ...
                         g_str_has_prefix(xname, "que_") || !strcmp(xname, "que") || g_str_has_prefix(xname, "resume"))
                {
...
static long get_desktop_index(GtkWindow* win)
{
    ...
    if (net_wm_desktop == None)
        g_fprintf(stderr, "spacefm: %s atom not found\n", atom_name);
    ...
    {
        if (type == None)
            g_fprintf(stderr, "spacefm: No such property from XGetWindowProperty() %s\n", atom_name);
        else if (data == NULL)
            g_fprintf(stderr, "spacefm: No data returned from XGetWindowProperty() %s\n", atom_name);
        else
            g_fprintf(stderr, "spacefm: XGetWindowProperty() %s failed\n", atom_name);
    }
    else
    {
        desktop = *data;
...
{ // find the last used spacefm window on the current desktop
    ...
    for (l = all_windows; l; l = l->next)
    {
        long desktop = get_desktop_index(GTK_WINDOW((FMMainWindow*)l->data));
...
const char* task_titles[] = {
    ...
    N_("Remain"),
    "StartTime"};
const char* task_names[] = {"task_col_status",
                            "task_col_count",
                            "task_col_path",
                            "task_col_file",
                            "task_col_to",
...
void main_context_fill(PtkFileBrowser* file_browser, XSetContext* c)
{
    ...
    if (!c->var[CONTEXT_NAME])
    {
        ...
        {
            c->var[CONTEXT_WRITE_ACCESS] =
                ptk_file_browser_no_access(c->var[CONTEXT_DIR], NULL) ? g_strdup("false") : g_strdup("true");
        }
        ...
        else
        {
            ...
            char* path = g_build_filename(c->var[CONTEXT_DIR], c->var[CONTEXT_NAME], NULL);
            c->var[CONTEXT_IS_DIR] =
                path && g_file_test(path, G_FILE_TEST_IS_DIR) ? g_strdup("true") : g_strdup("false");
    ...
    for (p = 1; p < 5; p++)
    {
        ...
        if (i != -1)
        {
            a_browser = (PtkFileBrowser*)gtk_notebook_get_nth_page(GTK_NOTEBOOK(main_window->panel[p - 1]), i);
        }
        else
            continue;
        ...
        if (file_browser == a_browser)
        {
            ...
            c->var[CONTEXT_TAB_COUNT] =
                g_strdup_printf("%d", gtk_notebook_get_n_pages(GTK_NOTEBOOK(main_window->panel[p - 1])));
        }
    }
    ...
    {
        GtkTreeIter it;
        GtkTreeModel* model_task = gtk_tree_view_get_model(GTK_TREE_VIEW(main_window->task_view));
        int task_count = 0;
...
void main_write_exports(VFSFileTask* vtask, const char* value, GString* buf)
{
    ...
    for (p = 1; p < 5; p++)
    {
        ...
        for (i = 0; i < num_pages; i++)
        {
            PtkFileBrowser* t_browser = PTK_FILE_BROWSER(gtk_notebook_get_nth_page(GTK_NOTEBOOK(main_window->panel[p - 1]), i));
...
{
    uint id = g_signal_lookup("columns-changed", G_TYPE_FROM_INSTANCE(view));
    if (id)
    {
...
void main_task_start_queued(GtkWidget* view, PtkFileTask* new_task)
{
    ...
    GSList* running = NULL;
    GSList* queued = NULL;
    gboolean smart = xset_get_b("task_q_smart");
    ...
    for (q = queued; q; q = q->next)
    {
        ...
        {
            PtkFileTask* rtask = (PtkFileTask*)r->data;
    ...
    if (!model || gtk_tree_model_get_iter_first(model, &it))
    {
        do
        {
            ...
            if (ptask && ptask->task && !ptask->complete &&
                (ptask->task->type != VFS_FILE_TASK_EXEC || ptask->task->exec_pid || job == JOB_STOP))
            {
                switch (job)
                {
                case JOB_STOP:
...
static gboolean idle_set_task_height(FMMainWindow* main_window)
{
    ...
    int taskh = xset_get_int("task_show_manager", "x"); // task height >=0.9.2
    if (taskh == 0)
    {
        // use pre-0.9.2 slider pos to calculate height
        int pos = xset_get_int("panel_sliders", "z"); // < 0.9.2 slider pos
        if (pos == 0)
            taskh = 200;
        else
...
void main_task_prepare_menu(FMMainWindow* main_window, GtkWidget* menu, GtkAccelGroup* accel_group)
{
    ...
    XSet* set = xset_set_cb("task_show_manager", on_task_popup_show, main_window);
...
gboolean on_task_button_press_event(GtkWidget* view, GdkEventButton* event, FMMainWindow* main_window)
{
    ...
    if (event->button == 3) // right click
    {
        ...
        xset_set_cb("font_task", main_update_fonts, file_browser);
        char* menu_elements =
            g_strdup_printf("task_stop sep_t3 task_pause task_que task_resume%s task_all sep_t4 task_show_manager "
...
void main_task_view_update_task(PtkFileTask* ptask)
{
    ...
    if (ptask->task->state_pause == VFS_FILE_TASK_RUNNING || ptask->pause_change_view)
    {
        ...
        const char* job_titles[] =
                {N_("moving"), N_("copying"), N_("deleting"), N_("linking"), N_("changing"), N_("running")};
        ...
        if (ptask->task->type != VFS_FILE_TASK_EXEC)
        {
            ...
            {
                status2 = g_strdup_printf(ngettext(_("%d error %s"), _("%d errors %s"), ptask->err_count),
                                          ptask->err_count,
...
GtkWidget* main_task_view_new(FMMainWindow* main_window)
{
    ...
    for (i = 0; i < 13; i++)
    {
        ...
        {
            // column width
            int width = xset_get_int(task_names[j], "y");
            if (width == 0)
                width = 80;
...
{
    return (!(value && value[0]) || !strcmp(value, "1") || !strcmp(value, "true") || !strcmp(value, "True") ||
            !strcmp(value, "TRUE") || !strcmp(value, "yes") || !strcmp(value, "Yes") || !strcmp(value, "YES"));
}
...
char main_window_socket_command(char* argv[], char** reply)
{
    ...
    const char* column_titles[] = {N_("Name"), N_("Size"), N_("Type"), N_("Permission"), N_("Owner"), N_("Modified")};
    ...
    *reply = NULL;
    ...
    if (!strcmp(argv[0], "set"))
    {
        ...
        else if (!strcmp(argv[i], "panel_hslider_top") || !strcmp(argv[i], "panel_hslider_bottom") ||
        ...
        else if (g_str_has_prefix(argv[i], "sort_"))
        {
            if (!strcmp(argv[i] + 5, "ascend"))
            {
                ptk_file_browser_set_sort_type(file_browser,
                                               bool(argv[i + 1]) ? GTK_SORT_ASCENDING : GTK_SORT_DESCENDING);
                return 0;
            }
        ...
        else if (!strcmp(argv[i], "clipboard_text") || !strcmp(argv[i], "clipboard_primary_text"))
        {
            ...
            GtkClipboard* clip =
                gtk_clipboard_get(!strcmp(argv[i], "clipboard_text") ? GDK_SELECTION_CLIPBOARD : GDK_SELECTION_PRIMARY);
        ...
        else if (!strcmp(argv[i], "clipboard_from_file") || !strcmp(argv[i], "clipboard_primary_from_file"))
        {
            ...
            {
                *reply = g_strdup_printf(_("spacefm: file '%s' does not contain valid UTF-8 text\n"), argv[i + 1]);
                g_free(str);
                return 2;
            }
            GtkClipboard* clip = gtk_clipboard_get(!strcmp(argv[i], "clipboard_from_file") ? GDK_SELECTION_CLIPBOARD
        ...
        else if (!strcmp(argv[i], "current_dir"))
        {
            ...
            {
                *reply = g_strdup_printf(_("spacefm: directory '%s' does not exist\n"), argv[i + 1]);
                return 1;
            }
                ...
                *reply =
                    g_strdup_printf("%d\n",
                                    xset_get_b_panel(file_browser->mypanel, "sort_extra") &&
                                            xset_get_int_panel(file_browser->mypanel, "sort_extra", "x") == XSET_B_TRUE
                                        ? 1
                                        : 0);
            else if (!strcmp(argv[i] + 5, "hidden_first"))
                *reply = g_strdup_printf(
    ...
    { // TASKNUM PROPERTY [VALUE]
        ...
        else if (!strcmp(argv[i + 1], "progress"))
        {
            ...
            j = TASK_COL_AVGEST;
        else if (!strcmp(argv[i + 1], "elapsed") || !strcmp(argv[i + 1], "started") || !strcmp(argv[i + 1], "status"))
        {
    ...
    { // TYPE [OPTIONS] ...
        ...
        if (!strcmp(argv[i], "cmd") || !strcmp(argv[i], "command"))
        {
            ...
            if (opt_task)
                *reply = g_strdup_printf("#!%s\n%s\n# Note: $new_task_id not valid until approx one half second after "
                                         "task start\nnew_task_window=%p\nnew_task_id=%p\n",
                                         BASHPATH,
                                         SHELL_SETTINGS,
                                         main_window,
        ...
        else if (!strcmp(argv[i], "mount") || !strcmp(argv[i], "unmount"))
        {
            ...
            else if (stat(real_path, &statbuf) == 0 && S_ISBLK(statbuf.st_mode))
            {
                ...
                device_file = g_strdup(real_path);
                     ...
                     ((real_path[0] != '/' && strstr(real_path, ":/")) || g_str_has_prefix(real_path, "//")))
            {
                // mount URL
                 ...
                 !strcmp(argv[i], "delete"))
        {
            ...
            if (!strcmp(argv[i], "copy"))
                ...
                return 1; // failsafe
            PtkFileTask* ptask = ptk_file_task_new(j,
                                                   l,
                                                   target_dir,
                                                   GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(file_browser))),
                                                   file_browser->task_view);
            ptk_file_task_run(ptask);
            *reply = g_strdup_printf("#!%s\n%s\n# Note: $new_task_id not valid until approx one half second after task "
                                     "start\nnew_task_window=%p\nnew_task_id=%p\n",
                                     BASHPATH,
                                     SHELL_SETTINGS,
                                     main_window,
    ...
    else if (!strcmp(argv[0], "add-event") || !strcmp(argv[0], "replace-event") || !strcmp(argv[0], "remove-event"))
    {
        XSet* set;
...
gboolean run_event(FMMainWindow* main_window, PtkFileBrowser* file_browser, XSet* preset, const char* event, int panel,
                   int tab, const char* focus, int keyval, int button, int state, gboolean visible, XSet* set,
                   char* ucmd)
{
    char* cmd;
    gboolean inhibit;
    ...
    if (!preset && (!strcmp(event, "evt_start") || !strcmp(event, "evt_exit") || !strcmp(event, "evt_device")))
    {
    ...
    if (set == evt_win_click)
    {
        replace = "ewptfbm";
        state = (state &
                 (GDK_SHIFT_MASK | GDK_CONTROL_MASK | GDK_MOD1_MASK | GDK_SUPER_MASK | GDK_HYPER_MASK | GDK_META_MASK));
    }
