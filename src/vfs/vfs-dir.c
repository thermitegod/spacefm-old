...
static void vfs_dir_monitor_callback(VFSFileMonitor* fm, VFSFileMonitorEvent event, const char* file_name,
                                     gpointer user_data);
...
VFSDir* vfs_dir_get_by_path(const char* path)
{
    ...
    if (G_UNLIKELY(!dir_hash))
    {
        ...
        if (0 == theme_change_notify)
            theme_change_notify =
                g_signal_connect(gtk_icon_theme_get_default(), "changed", G_CALLBACK(on_theme_changed), NULL);
    }
    else
    {
