...
#define HIDDEN_NON_BLOCK_FS                                                                               \
    "devpts proc fusectl pstore sysfs tmpfs devtmpfs ramfs aufs overlayfs cgroup binfmt_misc rpc_pipefs " \
    "fuse.gvfsd-fuse"
...
void info_drive_properties(device_t* device)
{
    ...
    {
        if (udev_device_get_property_value(device->udevice, drive_media_mapping[n].udev_property) == NULL)
            continue;
    ...
    {
        char* type = sysfs_get_string(device->native_path, "../../type");
        g_strstrip(type);
    ...
    if (device->device_is_media_available)
    {
        ...
        {
            if (udev_device_get_property_value(device->udevice, media_mapping[n].udev_property) == NULL)
                continue;
...
void info_device_properties(device_t* device)
{
    ...
    device->device_presentation_hide =
        g_strdup(udev_device_get_property_value(device->udevice, "UDISKS_PRESENTATION_HIDE"));
    device->device_presentation_nopolicy =
        g_strdup(udev_device_get_property_value(device->udevice, "UDISKS_PRESENTATION_NOPOLICY"));
    ...
    if ((device->id_usage && device->id_usage[0] != '\0') || (device->id_type && device->id_type[0] != '\0') ||
        (device->id_uuid && device->id_uuid[0] != '\0') || (device->id_label && device->id_label[0] != '\0'))
    {
    ...
    while (entry)
    {
        ...
        if (entry_name &&
            (g_str_has_prefix(entry_name, "/dev/disk/by-id/") || g_str_has_prefix(entry_name, "/dev/disk/by-uuid/")))
        {
...
char* info_mount_points(device_t* device)
{
    ...
    uint dmajor = MAJOR(device->devnum);
    uint dminor = MINOR(device->devnum);
...
void info_partition_table(device_t* device)
{
     ...
     */
    if ((value = udev_device_get_property_value(device->udevice, "UDISKS_PARTITION_TABLE")) && atoi(value) == 1)
    {
        device->partition_table_scheme =
            g_strdup(udev_device_get_property_value(device->udevice, "UDISKS_PARTITION_TABLE_SCHEME"));
        device->partition_table_count =
            g_strdup(udev_device_get_property_value(device->udevice, "UDISKS_PARTITION_TABLE_COUNT"));
    ...
    if (!is_partition_table)
    {
        ...
        char* s = g_path_get_basename(device->native_path);
...
void info_optical_disc(device_t* device)
{
    ...
    if (optical_state && atoi(optical_state) != 0)
    {
        ...
        device->optical_disc_num_tracks =
            g_strdup(udev_device_get_property_value(device->udevice, "ID_CDROM_MEDIA_TRACK_COUNT"));
        device->optical_disc_num_audio_tracks =
            g_strdup(udev_device_get_property_value(device->udevice, "ID_CDROM_MEDIA_TRACK_COUNT_AUDIO"));
        device->optical_disc_num_sessions =
            g_strdup(udev_device_get_property_value(device->udevice, "ID_CDROM_MEDIA_SESSION_COUNT"));
...
char* device_show_info(device_t* device)
{
    ...
    if (device->device_is_optical_disc)
    {
        ...
        line[i++] =
            g_strdup_printf("    num audio tracks:          %s\n",
...
void parse_mounts(gboolean report)
{
    ...
    if (report)
    {
        for (l = newmounts; l; l = l->next)
        {
            ...
            GList* found = g_list_find_custom(devmounts, (gconstpointer)devmount, (GCompareFunc)cmp_devmounts);
            if (found)
            {
                // g_printf("    found\n");
...
void vfs_volume_set_info(VFSVolume* volume)
{
    ...
    if (!fmt)
        parameter =
            g_strdup_printf("%s %s %s %s %s %s", disp_device, disp_size, disp_fstype, disp_label, disp_mount, disp_id);
    else
    {
        char* valuec;
...
{ // uses udev to read device parameters into returned volume
    ...
    volume->is_user_visible = device->device_presentation_hide ? !atoi(device->device_presentation_hide) : TRUE;
    volume->ever_mounted = FALSE;
    volume->open_main_window = NULL;
    volume->nopolicy = device->device_presentation_nopolicy ? atoi(device->device_presentation_nopolicy) : FALSE;
...
{ // read a non-block device
    ...
    else
    {
        ...
        if (!keep && !strstr(HIDDEN_NON_BLOCK_FS, mtab_fstype))
        {
            ...
            keep = g_str_has_prefix(point, g_get_user_cache_dir()) || g_str_has_prefix(point, "/media/") ||
                   g_str_has_prefix(point, "/run/media/") || g_str_has_prefix(mtab_fstype, "fuse.");
            if (!keep)
            {
                             ...
                             gboolean* run_in_terminal, char** mount_point)
{
            ...
            else
            {
                ...
                command =
                    replace_string(command,
                                   "%a",
                                   vol->is_mounted && vol->mount_point && vol->mount_point[0] ? vol->mount_point : "",
                                   FALSE);
                g_free(str);
            }
        }
    ...
    {    // also used for DEVICE_TYPE_OTHER unmount and prop
        ...
        if (strstr(command, "%a"))
        {
            ...
            else
            {
                ...
                command = replace_string(
                    command,
                    "%a",
                    vol && vol->is_mounted && vol->mount_point && vol->mount_point[0] ? vol->mount_point : "",
                    FALSE);
                g_free(str);
            }
        }
        ...
        if (netmount)
        {
            ...
            str = command;
            command = g_strdup_printf("fm_url_proto=%s; fm_url=%s; fm_url_host=%s; fm_url_port=%s; fm_url_user=%s; "
...
char* vfs_volume_device_unmount_cmd(VFSVolume* vol, gboolean* run_in_terminal)
{
    ...
    *run_in_terminal = FALSE;
    ...
    if (vol->device_type == DEVICE_TYPE_NETWORK)
    {
        ...
        {
            command =
                vfs_volume_handler_cmd(HANDLER_MODE_NET, HANDLER_UNMOUNT, vol, NULL, netmount, run_in_terminal, NULL);
            g_free(netmount->url);
            g_free(netmount->fstype);
    ...
    if (!command)
    {
        // discovery
        pointq =
            bash_quote(vol->device_type == DEVICE_TYPE_BLOCK || !vol->is_mounted ? vol->device_file : vol->mount_point);
        if ((s1 = g_find_program_in_path("udevil")))
        {
            // udevil
...
char* vfs_volume_get_mount_options(VFSVolume* vol, char* options)
{
    ...
    while (ptr[0] != '\0')
    {
        ...
        char* single = g_strndup(ptr, comma - ptr);
        if (!strchr(single, '+') && !strchr(single, '-'))
        {
            ...
            {
                // add option
                g_strlcpy(newoptr, single, sizeof(newoptr));
        ...
        else if ((plus = strchr(single, '+')))
        {
            ...
            char* singlefs = g_strdup_printf("%s+%s", single, fstype);
...
void vfs_volume_automount(VFSVolume* vol)
{
    if (vol->is_mounted || vol->ever_mounted
        ...
        || vol->is_audiocd
#endif
        || vol->should_autounmount || !vfs_volume_is_automount(vol))
        return;
        ...
        if (fstype && fstype[0])
        {
            ...
            if ((ptr = xset_get_s("dev_change")))
            {
                while (ptr[0])
                {
                    while (ptr[0] == ' ' || ptr[0] == ',')
                        ptr++;
                    if (g_str_has_prefix(ptr, fstype) && (ptr[len] == ' ' || ptr[len] == ',' || ptr[len] == '\0'))
                    {
