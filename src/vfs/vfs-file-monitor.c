...
static gboolean on_inotify_event(GIOChannel* channel, GIOCondition cond, gpointer user_data);
...
static gboolean connect_to_inotify()
{
    ...
    inotify_io_watch =
        g_io_add_watch(inotify_io_channel, G_IO_IN | G_IO_PRI | G_IO_HUP | G_IO_ERR, on_inotify_event, NULL);
    return TRUE;
}
        ...
        if (monitor->wd < 0)
        {
            ...
            case ENOSPC:
                msg = "ENOSPC The user limit on the total number of inotify watches (cat "
                      "/proc/sys/fs/inotify/max_user_watches) was reached or the kernel failed to allocate a needed "
                      "resource.";
                break;
            default:
                msg = "??? Unknown error.";
...
#define BUF_LEN (1024 * (sizeof(struct inotify_event) + 16))
    ...
    if (cond & (G_IO_HUP | G_IO_ERR))
    {
        ...
        if (g_hash_table_size(monitor_hash) > 0)
        {
            ...
            */
            if (connect_to_inotify())
                g_hash_table_foreach(monitor_hash, (GHFunc)reconnect_inotify, NULL);
        }
    ...
    while (i < len)
    {
        ...
        monitor = (VFSFileMonitor*)g_hash_table_find(monitor_hash, find_monitor, GINT_TO_POINTER(ievent->wd));
        if (G_LIKELY(monitor))
        {
