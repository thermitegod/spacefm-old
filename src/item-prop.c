...
static const char* context_sub_list[] =
{
    ...
    "6%%%%%archive_types || .gz || .bz2 || .7z || .xz || .zst || .txz || .tgz || .zip || .rar || .tar || .tar.gz || .tar.xz || .tar.zst || .tar.bz2 || .tar.7z%%%%%audio_types || .mp3 || .MP3 || .m3u || .wav || .wma || .aac || .ac3 || .flac || .ram || .m4a || .ogg%%%%%image_types || .jpg || .jpeg || .gif || .png || .xpm%%%%%video_types || .mp4 || .MP4 || .avi || .AVI || .mkv || .mpeg || .mpg || .flv || .vob || .asf || .rm || .m2ts || .mov",  //"Filename",
    "0%%%%%",  //"Dir",
    ...
    "0%%%%%",  //"Device UDI",
#ifdef DEPRECATED_HW
    "2%%%%%audiocd%%%%%blank%%%%%dvd%%%%%dvd && blank%%%%%ejectable%%%%%internal%%%%%mountable%%%%%mounted%%%%%no_media%%%%%optical%%%%%optical && blank%%%%%optical && mountable%%%%%optical && mounted%%%%%removable%%%%%removable && mountable%%%%%removable && mounted%%%%%removable || optical%%%%%table%%%%%policy_hide%%%%%policy_noauto",  //"Device Properties",
#else
    "2%%%%%ejectable%%%%%internal%%%%%mountable%%%%%mounted%%%%%no_media%%%%%removable%%%%%removable && mountable%%%%%removable && mounted%%%%%table%%%%%policy_hide%%%%%policy_noauto",  //"Device Properties",
#endif
    "8%%%%%0%%%%%1%%%%%2",  //"Task Count",
    "0%%%%%",  //"Task Dir",
...
#if (GTK_MAJOR_VERSION == 3)
    gtk_box_pack_start( GTK_BOX( hbox_btns ),
                        GTK_WIDGET(gtk_separator_new(GTK_ORIENTATION_VERTICAL)), FALSE, TRUE, 4);
#elif (GTK_MAJOR_VERSION == 2)
    gtk_box_pack_start( GTK_BOX( hbox_btns ),
