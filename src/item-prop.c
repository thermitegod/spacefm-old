...
static const char* context_sub[] = {N_("MIME Type"),
                                    N_("Filename"),
                                    N_("Directory"),
                                    N_("Dir Write Access"),
                                    N_("File Is Text"),
...
static const char* context_sub_list[] =
    {
        "4%%%%%application/%%%%%audio/%%%%%audio/ || "
        "video/%%%%%image/%%%%%inode/directory%%%%%text/%%%%%video/%%%%%application/x-bzip||application/"
        "x-bzip-compressed-tar||application/x-gzip||application/zip||application/x-7z-compressed||application/"
        "x-bzip2||application/x-bzip2-compressed-tar||application/x-xz-compressed-tar||application/"
        "x-compressed-tar||application/x-rar", //"MIME Type",
        "6%%%%%archive_types || .gz || .bz2 || .7z || .xz || .zst || .txz || .tgz || .zip || .rar || .tar || .tar.gz "
        "|| .tar.xz || .tar.zst || .tar.bz2 || .tar.7z%%%%%audio_types || .mp3 || .MP3 || .m3u || .wav || .wma || .aac "
        "|| .ac3 || .flac || .ram || .m4a || .ogg%%%%%image_types || .jpg || .jpeg || .gif || .png || "
        ".xpm%%%%%video_types || .mp4 || .MP4 || .avi || .AVI || .mkv || .mpeg || .mpg || .flv || .vob || .asf || .rm "
        "|| .m2ts || .mov",                                         //"Filename",
        "0%%%%%",                                                   //"Dir",
        ...
        "0%%%%%",                                                   //"Device Label",
        "0%%%%%ext2%%%%%ext3%%%%%ext4%%%%%ext2 || ext3 || ext4%%%%%ntfs%%%%%swap%%%%%ufs%%%%%vfat%%%%%xfs", // Device
                                                                                                            // FSType",
        "0%%%%%", //"Device UDI",
#ifdef DEPRECATED_HW
        "2%%%%%audiocd%%%%%blank%%%%%dvd%%%%%dvd && "
        "blank%%%%%ejectable%%%%%internal%%%%%mountable%%%%%mounted%%%%%no_media%%%%%optical%%%%%optical && "
        "blank%%%%%optical && mountable%%%%%optical && mounted%%%%%removable%%%%%removable && mountable%%%%%removable "
        "&& mounted%%%%%removable || optical%%%%%table%%%%%policy_hide%%%%%policy_noauto", //"Device Properties",
#else
        "2%%%%%ejectable%%%%%internal%%%%%mountable%%%%%mounted%%%%%no_media%%%%%removable%%%%%removable && "
        "mountable%%%%%removable && mounted%%%%%table%%%%%policy_hide%%%%%policy_noauto", //"Device Properties",
#endif
        "8%%%%%0%%%%%1%%%%%2",                                        //"Task Count",
        "0%%%%%",                                                     //"Task Dir",
...
static const char* context_comp[] = {N_("equals"),
                                     N_("doesn't equal"),
                                     N_("contains"),
                                     N_("doesn't contain"),
                                     N_("begins with"),
...
void enable_context(ContextData* ctxt)
{
    // GtkTreeIter it;
    gboolean is_sel =
        gtk_tree_selection_get_selected(gtk_tree_view_get_selection(GTK_TREE_VIEW(ctxt->view)), NULL, NULL);
...
void on_context_entry_insert(GtkEntryBuffer* buf, uint position, char* chars, uint n_chars, gpointer user_data)
...
void on_script_toggled(GtkWidget* item, ContextData* ctxt)
{
    ...
    else
    {
        ...
        char* str =
            g_strdup_printf("%s  $fm_cmd_dir  %s", _("Command Dir"), dir_has_files(path) ? "" : _("(no files)"));
    ...
    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ctxt->cmd_opt_line)))
    {
        ...
        if (!(path && mime_type_is_text_file(path, NULL)))
        {
            xset_msg_dialog(GTK_WIDGET(ctxt->dlg),
                            ...
                            NULL,
                            0,
                            _("The command line does not begin with a text file (script) to be opened, or the script "
    ...
    else
    {
        ...
        if (widget == ctxt->item_choose)
        {
            ...
            VFSMimeType* mime_type =
                vfs_mime_type_get_from_type(ctxt->context->var[CONTEXT_MIME] && ctxt->context->var[CONTEXT_MIME][0]
                                                ? ctxt->context->var[CONTEXT_MIME]
                                                : XDG_MIME_TYPE_UNKNOWN);
            char* app = (char*)ptk_choose_app_for_mime_type(GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(ctxt->dlg))),
                                                            mime_type,
                                                            TRUE,
                                                            FALSE,
                                                            FALSE,
        ...
        else
        {
            // Browse
            char* exec_path =
                xset_file_dialog(ctxt->dlg, GTK_FILE_CHOOSER_ACTION_OPEN, _("Choose Executable"), "/usr/bin", NULL);
...
void replace_item_props(ContextData* ctxt)
{
        ...
        rset->tool <= XSET_TOOL_CUSTOM)
    {
        ...
        if (x == XSET_CMD_LINE)
        {
            ...
            if (rset->line && strlen(rset->line) > 2000)
                xset_msg_dialog(ctxt->dlg,
                                ...
                                NULL,
                                GTK_BUTTONS_OK,
                                _("Your command line is greater than 2000 characters and may be truncated when saved.  "
                                  "Consider using a command script instead by selecting Script on the Command tab."),
                                NULL,
                                NULL);
        }
        else
        ...
        // run options
        mset->in_terminal = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ctxt->opt_terminal)) && !is_bookmark_or_app
                                ? XSET_B_TRUE
                                : XSET_B_UNSET;
        mset->keep_terminal =
            gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ctxt->opt_keep_term)) && !is_bookmark_or_app ? XSET_B_TRUE
                                                                                                        : XSET_B_UNSET;
        mset->task = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ctxt->opt_task)) && !is_bookmark_or_app
                         ? XSET_B_TRUE
                         : XSET_B_UNSET;
        mset->task_pop = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ctxt->opt_task_pop)) && !is_bookmark_or_app
                             ? XSET_B_TRUE
                             : XSET_B_UNSET;
        mset->task_err = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ctxt->opt_task_err)) && !is_bookmark_or_app
                             ? XSET_B_TRUE
                             : XSET_B_UNSET;
        mset->task_out = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ctxt->opt_task_out)) && !is_bookmark_or_app
                             ? XSET_B_TRUE
                             : XSET_B_UNSET;
        mset->scroll_lock = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(ctxt->opt_scroll)) || is_bookmark_or_app
                                ? XSET_B_UNSET
                                : XSET_B_TRUE;
    ...
    if (rset->menu_style != XSET_MENU_SEP && !rset->plugin)
    {
        ...
        if (rset->tool > XSET_TOOL_CUSTOM &&
            !g_strcmp0(gtk_entry_get_text(GTK_ENTRY(ctxt->item_name)), xset_get_builtin_toolitem_label(rset->tool)))
...
void on_prop_notebook_switch_page(GtkNotebook* notebook, GtkWidget* page, uint page_num, ContextData* ctxt)
{
    GtkWidget* widget;
    if (page_num == 0)
...
static void on_entry_buffer_inserted_text(GtkEntryBuffer* buf, uint position, char* chars, uint n_chars,
                                          ContextData* ctxt)
{
...
static void on_entry_buffer_deleted_text(GtkEntryBuffer* buf, uint position, uint n_chars, ContextData* ctxt)
{
...
void xset_item_prop_dlg(XSetContext* context, XSet* set, int page)
{
    ...
    // Dialog
    ctxt->dlg = gtk_dialog_new_with_buttons(set->tool ? _("Toolbar Item Properties") : _("Menu Item Properties"),
    ...
    g_signal_connect(G_OBJECT(ctxt->current_value),
                     ...
                     ctxt);
    g_signal_connect_after(G_OBJECT(gtk_entry_get_buffer(GTK_ENTRY(gtk_bin_get_child(GTK_BIN(ctxt->box_value))))),
                           "inserted-text",
...
#if (GTK_MAJOR_VERSION == 3)
    gtk_box_pack_start(GTK_BOX(hbox_btns), GTK_WIDGET(gtk_separator_new(GTK_ORIENTATION_VERTICAL)), FALSE, TRUE, 4);
#elif (GTK_MAJOR_VERSION == 2)
    ...
    ctxt->cmd_opt_script =
        gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(ctxt->cmd_opt_line), _("_Script"));
    ...
    ctxt->cmd_opt_checkbox =
        gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(ctxt->cmd_opt_normal), _("Checkbox"));
    ctxt->cmd_opt_confirm =
        gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(ctxt->cmd_opt_normal), _("Confirmation"));
    ctxt->cmd_opt_input =
        gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(ctxt->cmd_opt_normal), _("Input"));
    ...
    path = g_build_filename(xset_get_config_dir(), "plugin-data", rset->plugin ? mset->name : rset->name, NULL);
    ...
    if (set->tool > XSET_TOOL_CUSTOM)
        item_type_str =
            g_strdup_printf("%s: %s", _("Built-In Toolbar Item"), xset_get_builtin_toolitem_label(set->tool));
    ...
    gtk_widget_set_sensitive(ctxt->icon_choose_btn,
                             rset->menu_style != XSET_MENU_RADIO && rset->menu_style != XSET_MENU_SEP);
