...
static void set_element_value(CustomElement* el, const char* name, char* value)
{
    ...
    case CDLG_WINDOW_ICON:
        if (value[0] != '\0')
            pixbuf =
                gtk_icon_theme_load_icon(gtk_icon_theme_get_default(), value, 16, GTK_ICON_LOOKUP_USE_BUILTIN, NULL);
        else
            pixbuf = NULL;
    ...
    case CDLG_FREE_BUTTON:
        if (el_name->widgets->next && (w = GTK_WIDGET(el_name->widgets->next->data)))
        {
            ...
            if (!sep && (!g_strcmp0(el_name->val, "ok") || !g_strcmp0(el_name->val, "cancel") ||
    ...
    case CDLG_EDITOR:
        ...
        if (el_name->type == CDLG_VIEWER && el_name->widgets->next)
        {
            ...
            if (el_name->option)
            {
                ...
                GtkAdjustment* adj =
                    gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(el_name->widgets->next->next->data));
...
static void internal_command(CustomElement* el, int icmd, GList* args, char* xvalue)
{
    ...
    if (icmd != CMD_NOOP && icmd != CMD_CLOSE && icmd != CMD_SOURCE && icmd != CMD_FOCUS && icmd != CMD_SHOW && !cname)
    {
    ...
    if (icmd == CMD_SET && (!strcmp(cname, "title") || !strcmp(cname, "windowtitle") || !strcmp(cname, "windowsize") ||
    ...
    if (icmd != CMD_NOOP && icmd != CMD_SOURCE && icmd != CMD_CLOSE && cname && !(el_name = el_from_name(el, cname)))
    {
        if (cname[0] != '\0')
    ...
    */
    ...
    if (g_io_channel_read_chars(channel, line, sizeof(line), &size, NULL) == G_IO_STATUS_NORMAL && size > 0)
    {
        ...
        else if (el->type == CDLG_VIEWER && el->widgets->next)
        {
            ...
            {
                GtkAdjustment* adj =
                    gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(el->widgets->next->next->data));
                if (gtk_adjustment_get_upper(adj) - gtk_adjustment_get_value(adj) <
                    gtk_adjustment_get_page_size(adj) + 40)
...
static void cb_file_value_change(VFSFileMonitor* fm, VFSFileMonitorEvent event, const char* file_name,
                                 CustomElement* el)
{
...
static void write_source(GtkWidget* dlg, CustomElement* el_pressed, FILE* out, gboolean is_cancel)
{
    ...
    g_fprintf(out,
              ...
              "\"$(spacefm --dialog --label \"Message\" --button ok)\"\n\n",
              BASHPATH,
              SHELL_SETTINGS);
    if (!el_pressed)
    {
    ...
    for (l = elements; l; l = l->next)
    {
        ...
        case CDLG_PASSWORD:
            ...
            if (!is_cancel && el->args && ((char*)el->args->data)[0] == '@')
            {
                ...
                if (el->update_timeout)
                    g_source_remove(el->update_timeout);
                el->update_timeout =
                    g_timeout_add_full(G_PRIORITY_DEFAULT_IDLE, 300, (GSourceFunc)delayed_update_false, el, NULL);
        ...
        case CDLG_RADIO:
            ...
            write_value(out,
                        ...
                        el->name,
                        NULL,
                        gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(el->widgets->next->data)) ? "1" : "0");
            // save file
            if (!is_cancel && el->args && el->args->next && ((char*)el->args->next->data)[0] == '@')
            {
...
{
    int keymod = (evt->state & (GDK_SHIFT_MASK | GDK_CONTROL_MASK | GDK_MOD1_MASK | GDK_SUPER_MASK | GDK_HYPER_MASK |
                                GDK_META_MASK));
...
static void update_element(CustomElement* el, GtkWidget* box, GSList** radio, int pad)
{
    ...
    while (args && g_str_has_prefix((char*)args->data, "--"))
    {
        if (!strcmp((char*)args->data, "--font"))
        {
            ...
            expand = TRUE;
        else if ((el->type == CDLG_INPUT || el->type == CDLG_INPUT_LARGE) && !strcmp((char*)args->data, "--select") &&
                 args->next)
        {
            args = args->next;
    ...
    case CDLG_FREE_BUTTON:
        ...
        if (el->widgets->next && (w = GTK_WIDGET(el->widgets->next->data)))
        {
            if (el->val && (sep = strrchr(el->val, ':')))
                ...
                sep = NULL;
            gtk_button_set_image(GTK_BUTTON(w), NULL);
            if (!sep && (!g_strcmp0(el->val, "ok") || !g_strcmp0(el->val, "cancel") || !g_strcmp0(el->val, "close") ||
                         !g_strcmp0(el->val, "open") || !g_strcmp0(el->val, "yes") || !g_strcmp0(el->val, "no") ||
                         !g_strcmp0(el->val, "apply") || !g_strcmp0(el->val, "delete") || !g_strcmp0(el->val, "edit") ||
                         !g_strcmp0(el->val, "save") || !g_strcmp0(el->val, "help") || !g_strcmp0(el->val, "stop")))
            {
                // stock button
                gtk_button_set_use_stock(GTK_BUTTON(w), TRUE);
    ...
    case CDLG_IMAGE:
        ...
        // destroy old image
        if (el->widgets->next && el->widgets->next->next && (w = GTK_WIDGET(el->widgets->next->next->data)))
        {
            gtk_widget_destroy(w);
    ...
    case CDLG_PASSWORD:
        ...
        else if (el->widgets->next && args && ((char*)args->data)[0] == '@')
        {
            ...
            if (g_strcmp0(str, el->val))
            {
                ...
                if (el->type == CDLG_INPUT_LARGE)
                {
                    gtk_text_buffer_set_text(gtk_text_view_get_buffer(GTK_TEXT_VIEW(el->widgets->next->data)),
                                             el->val ? el->val : "",
                                             -1);
    ...
    case CDLG_EDITOR:
        ...
        if (args && ((char*)args->data)[0] != '\0' && el->type == CDLG_VIEWER && el->widgets->next)
        {
            ...
            if (selstart && stat((char*)args->data, &statbuf) != -1 && S_ISFIFO(statbuf.st_mode))
            {
                // watch pipe
        ...
        else if (args && ((char*)args->data)[0] != '\0' && selstart && el->type == CDLG_EDITOR && el->widgets->next)
        {
            // new editor
            buf = gtk_text_view_get_buffer(GTK_TEXT_VIEW(el->widgets->next->data));
    ...
    case CDLG_COMMAND:
        if (!el->option && args)
        {
            if (((char*)args->data)[0] != '\0' && stat((char*)args->data, &statbuf) != -1 && S_ISFIFO(statbuf.st_mode))
            {
                // watch pipe
                GIOChannel* channel = g_io_channel_new_file((char*)args->data, "r+", NULL);
                if (channel)
    ...
    case CDLG_MLIST:
        if (!el->widgets->next && box)
        {
            ...
            gtk_tree_selection_set_mode(tree_sel,
                                        el->type == CDLG_MLIST ? GTK_SELECTION_MULTIPLE : GTK_SELECTION_SINGLE);
            GtkWidget* scroll = gtk_scrolled_window_new(NULL, NULL);
            gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scroll), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
            gtk_container_add(GTK_CONTAINER(scroll), w);
            gtk_box_pack_start(GTK_BOX(box), GTK_WIDGET(scroll), !compact, TRUE, pad);
    ...
    case CDLG_CHOOSER:
        ...
        {
            if (chooser_dir)
                i = chooser_save ? GTK_FILE_CHOOSER_ACTION_CREATE_FOLDER : GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER;
...
static void build_dialog(GList* elements)
{
    ...
    gtk_window_set_position(GTK_WINDOW(dlg), GTK_WIN_POS_CENTER);
    GdkPixbuf* pixbuf =
        gtk_icon_theme_load_icon(gtk_icon_theme_get_default(), DEFAULT_ICON, 16, GTK_ICON_LOOKUP_USE_BUILTIN, NULL);
    if (pixbuf)
        gtk_window_set_icon(GTK_WINDOW(dlg), pixbuf);
    ...
    for (l = elements; l; l = l->next)
    {
        ...
        if (!is_large && el->widgets->next &&
            (el->type == CDLG_CHOOSER || el->type == CDLG_MLIST || el->type == CDLG_EDITOR || el->type == CDLG_VIEWER ||
             el->type == CDLG_LIST))
            is_large = TRUE;
    }
    g_list_free(boxes);
