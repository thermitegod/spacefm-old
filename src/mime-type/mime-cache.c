 ...
 */
static const char* lookup_reverse_suffix_nodes(const char* buf, const char* nodes, guint32 n, const char* name,
...
const char* mime_cache_lookup_suffix(MimeCache* cache, const char* filename, const char** suffix_pos)
{
    ...
    else /* before mime.cache ver: 1.1 */
    {
        ...
        for (i = 0; i < n; ++i, root += 16)
        {
            ...
            {
                mime_type =
                    lookup_suffix_nodes(cache->buffer, cache->buffer + first_child_off, n, g_utf8_next_char(suffix));
