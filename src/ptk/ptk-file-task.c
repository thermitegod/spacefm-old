...
static gboolean on_vfs_file_task_state_cb(VFSFileTask* task, VFSFileTaskState state, gpointer state_data,
                                          gpointer user_data);
...
PtkFileTask* ptk_file_task_new(VFSFileTaskType type, GList* src_files, const char* dest_dir, GtkWindow* parent_window,
                               GtkWidget* task_view)
{
    // g_printf("ptk_file_task_new\n");
    ...
    */
    ...
    // g_printf("GUI_THREAD = %#x\n", self );
    // g_printf("ptk_file_task_new DONE ptask=%#x\n", ptask);
    return ptask;
}
...
//g_printf("ptk_file_task_destroy_delayed %d\n", task->keep_dlg);
...
{
    // g_printf("ptk_file_task_destroy ptask=%#x\n", ptask);
    if (ptask->timeout)
    {
    ...
    if (ptask->task->type == VFS_FILE_TASK_EXEC)
    {
        // g_printf("    g_io_channel_shutdown\n");
        ...
        // g_printf("    g_io_channel_shutdown DONE\n");
    }
    ...
    */
    ...
    // g_printf("ptk_file_task_destroy DONE ptask=%#x\n", ptask);
}
...
gboolean on_progress_timer(PtkFileTask* ptask)
{
    ...
    if (++ptask->progress_count < 6)
        return TRUE;
    ptask->progress_count = 0;
    // g_printf("on_progress_timer ptask=%p\n", ptask);
    ...
    if (ptask->complete)
    {
        if (!ptask->progress_dlg || (!ptask->err_count && !ptask->keep_dlg))
        {
            ...
            // g_printf("on_progress_timer DONE FALSE-COMPLETE ptask=%p\n", ptask);
            return FALSE;
        }
        else if (ptask->progress_dlg && ptask->err_count)
            gtk_window_present(GTK_WINDOW(ptask->progress_dlg));
    }
    // g_printf("on_progress_timer DONE TRUE ptask=%p\n", ptask);
    return !ptask->complete;
}

gboolean ptk_file_task_add_main(PtkFileTask* ptask)
{
    // g_printf("ptk_file_task_add_main ptask=%#x\n", ptask);
    if (ptask->timeout)
    {
        g_source_remove(ptask->timeout);
        ptask->timeout = 0;
    ...
    // g_printf("ptk_file_task_add_main DONE ptask=%#x\n", ptask);
    return FALSE;
}
...
{
    // g_printf("ptk_file_task_run ptask=%#x\n", ptask);
    ...
    // g_printf("ptk_file_task_run DONE ptask=%#x\n", ptask);
}
    ...
    if (ptask->task->type == VFS_FILE_TASK_EXEC)
    {
        ...
        if (ptask->task->exec_pid)
        {
            ...
            if (ptask->task->exec_as_user && geteuid() != 0 && (gsu = get_valid_gsu()))
            {
                ...
                PtkFileTask* ptask2 =
                    ptk_file_exec_new(_("Kill As Other"), NULL, GTK_WIDGET(ptask->parent_window), ptask->task_view);
    ...
    if (ptask->task->type == VFS_FILE_TASK_EXEC)
    {
        ...
        if (sig && ptask->task->exec_pid)
        {
            ...
            if (ptask->task->exec_as_user && geteuid() != 0 && (gsu = get_valid_gsu()))
            {
                ...
                PtkFileTask* ptask2 = ptk_file_exec_new(sig == SIGSTOP ? _("Stop As Other") : _("Cont As Other"),
                                                        NULL,
...
void set_progress_icon(PtkFileTask* ptask)
{
    ...
    if (task->state_pause != VFS_FILE_TASK_RUNNING)
        ...
        pixbuf = gtk_icon_theme_load_icon(gtk_icon_theme_get_default(), "error", 16, GTK_ICON_LOOKUP_USE_BUILTIN, NULL);
    else if (task->type == VFS_FILE_TASK_MOVE || task->type == VFS_FILE_TASK_COPY || task->type == VFS_FILE_TASK_LINK)
        pixbuf =
            gtk_icon_theme_load_icon(gtk_icon_theme_get_default(), "stock_copy", 16, GTK_ICON_LOOKUP_USE_BUILTIN, NULL);
...
void ptk_file_task_progress_open(PtkFileTask* ptask)
{
    ...
    // g_printf("ptk_file_task_progress_open\n");

    VFSFileTask* task = ptask->task;

    ptask->progress_dlg =
        gtk_dialog_new_with_buttons(_(titles[task->type]), NULL /*was task->parent_window*/, 0, NULL, NULL);
    ...
    */  //MOD
    ...
    if (task->type != VFS_FILE_TASK_EXEC)
    {
        ...
        gboolean overtask =
            task->type == VFS_FILE_TASK_MOVE || task->type == VFS_FILE_TASK_COPY || task->type == VFS_FILE_TASK_LINK;
        ...
        gtk_combo_box_set_active(GTK_COMBO_BOX(ptask->error_combo),
                                 ptask->err_mode < G_N_ELEMENTS(error_options) ? ptask->err_mode : 0);
    ...
    ptask->progress_count = 50; // trigger fast display
    // g_printf("ptk_file_task_progress_open DONE\n");
}
...
void ptk_file_task_progress_update(PtkFileTask* ptask)
{
    ...
    // g_printf("ptk_file_task_progress_update ptask=%p\n", ptask);
    ...
    */
    ...
    if (ptask->log_appended || ptask->force_scroll)
    {
        if (!task->exec_scroll_lock)
        {
            ...
            if (ptask->force_scroll ||
                gtk_adjustment_get_upper(adj) - gtk_adjustment_get_value(adj) < gtk_adjustment_get_page_size(adj) + 40)
            {
                // g_printf("    scroll to end line %d\n", ptask->log_end, gtk_text_buffer_get_line_count(
                // ptask->log_buf ));
                gtk_text_view_scroll_to_mark(GTK_TEXT_VIEW(ptask->error_view), ptask->log_end, 0.0, FALSE, 0, 0);
            }
        }
    ...
    if (ptask->complete)
    {
        if (ptask->aborted)
        {
            if (task->err_count && task->type != VFS_FILE_TASK_EXEC)
            {
                if (ptask->err_mode == PTASK_ERROR_FIRST)
                    ...
                    errs = g_strdup_printf(ngettext("Stopped with %d error", "Stopped with %d errors", task->err_count),
                                           task->err_count);
            }
            else
        ...
        else
        {
            if (task->type != VFS_FILE_TASK_EXEC)
            {
                if (task->err_count)
                    errs =
                        g_strdup_printf(ngettext("Finished with %d error", "Finished with %d errors", task->err_count),
                                        task->err_count);
                else
    ...
    else
    {
        ...
        {
            if (task->err_count)
                errs = g_strdup_printf(ngettext("Running with %d error", "Running with %d errors", task->err_count),
                                       task->err_count);
            else
    ...
    g_free(errs);
    // g_printf("ptk_file_task_progress_update DONE ptask=%#x\n", ptask);
}
...
void ptk_file_task_update(PtkFileTask* ptask)
{
    ...
    if (task->type == VFS_FILE_TASK_EXEC)
    {
        ...
        if (!ptask->complete && task->exec_pid && waitpid(task->exec_pid, &status, WNOHANG))
        {
            ...
            g_printf("child ZOMBIED  pid=%d exit_status=%d\n", task->exec_pid, task->exec_exit_status);
            task->exec_pid = 0;
            ptask->complete = TRUE;
        }
    }
    ...
    if (gtk_text_buffer_get_char_count(task->add_log_buf))
    {
            ...
            gtk_text_buffer_get_line_count(ptask->log_buf) > 800)
        {
            ...
            gtk_text_buffer_get_start_iter(ptask->log_buf, &siter);
            if (task->type == VFS_FILE_TASK_EXEC)
                gtk_text_buffer_insert(ptask->log_buf,
                                       &siter,
...
gboolean on_vfs_file_task_state_cb(VFSFileTask* task, VFSFileTaskState state, gpointer state_data, gpointer user_data)
{
    ...
    case VFS_FILE_TASK_ERROR:
        ...
        else if (ptask->err_mode == PTASK_ERROR_ANY || (task->error_first && ptask->err_mode == PTASK_ERROR_FIRST))
        {
            ret = FALSE;
            ptask->aborted = TRUE;
        }
...
void query_overwrite_response(GtkDialog* dlg, gint response, PtkFileTask* ptask)
{
    ...
    {
        ptk_file_task_lock(ptask);
        ptask->query_ret = (response != GTK_RESPONSE_DELETE_EVENT) && (response != GTK_RESPONSE_CANCEL);
...
static void query_overwrite(PtkFileTask* ptask)
{
    ...
    if (different_files && is_dest_dir == is_src_dir)
    {
        ...
        else
        {
            ...
            from_size_str =
                g_strdup_printf("\t%s\t%s%s%s%s", src_time, src_size, src_rel ? "\t" : "", src_rel, src_link);
