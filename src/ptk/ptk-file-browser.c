...
static void ptk_file_browser_get_property(GObject* obj, uint prop_id, GValue* value, GParamSpec* pspec);
static void ptk_file_browser_set_property(GObject* obj, uint prop_id, const GValue* value, GParamSpec* pspec);
...
static void on_folder_view_row_activated(GtkTreeView* tree_view, GtkTreePath* path, GtkTreeViewColumn* col,
                                         PtkFileBrowser* file_browser);
static void on_folder_view_item_sel_change(ExoIconView* iconview, PtkFileBrowser* file_browser);

static gboolean on_folder_view_button_press_event(GtkWidget* widget, GdkEventButton* event,
                                                  PtkFileBrowser* file_browser);
static gboolean on_folder_view_button_release_event(GtkWidget* widget, GdkEventButton* event,
                                                    PtkFileBrowser* file_browser);
...
/* Drag & Drop */
static void on_folder_view_drag_data_received(GtkWidget* widget, GdkDragContext* drag_context, int x, int y,
                                              GtkSelectionData* sel_data, uint info, uint time, gpointer user_data);

static void on_folder_view_drag_data_get(GtkWidget* widget, GdkDragContext* drag_context, GtkSelectionData* sel_data,
                                         uint info, uint time, PtkFileBrowser* file_browser);

static void on_folder_view_drag_begin(GtkWidget* widget, GdkDragContext* drag_context, PtkFileBrowser* file_browser);

static gboolean on_folder_view_drag_motion(GtkWidget* widget, GdkDragContext* drag_context, int x, int y, uint time,
                                           PtkFileBrowser* file_browser);

static gboolean on_folder_view_drag_leave(GtkWidget* widget, GdkDragContext* drag_context, uint time,
                                          PtkFileBrowser* file_browser);

static gboolean on_folder_view_drag_drop(GtkWidget* widget, GdkDragContext* drag_context, int x, int y, uint time,
                                         PtkFileBrowser* file_browser);
...
static uint folder_view_auto_scroll_timer = 0;
...
static void g_cclosure_marshal_VOID__POINTER_POINTER(GClosure* closure, GValue* return_value G_GNUC_UNUSED,
...
static void g_cclosure_marshal_VOID__POINTER_INT(GClosure* closure, GValue* return_value, uint n_param_values,
                                                 const GValue* param_values, gpointer invocation_hint,
                                                 gpointer marshal_data)
{
    typedef void (*GMarshalFunc_VOID__POINTER_INT)(gpointer data1, gpointer arg_1, int arg_2, gpointer data2);
...
void ptk_file_browser_class_init(PtkFileBrowserClass* klass)
{
    ...
    signals[CONTENT_CHANGE_SIGNAL] = g_signal_new("content-change",
                                                  G_TYPE_FROM_CLASS(klass),
                                                  G_SIGNAL_RUN_LAST,
    ...
    signals[PANE_MODE_CHANGE_SIGNAL] = g_signal_new("pane-mode-change",
                                                    G_TYPE_FROM_CLASS(klass),
                                                    G_SIGNAL_RUN_LAST,
...
void on_address_bar_activate(GtkWidget* entry, PtkFileBrowser* file_browser)
{
    ...
    char* final_path = vfs_file_resolve_path(ptk_file_browser_get_cwd(file_browser), text);
    ...
    else
    {
        ...
        else if (final_path_exists)
        {
            ...
            else
            {
                ...
                char* dir_path = g_path_get_dirname(final_path);
...
#if (GTK_MAJOR_VERSION == 3)
        if (scolor && gdk_rgba_parse(scolor, &color))
            gtk_widget_override_background_color(GTK_WIDGET(file_browser->status_frame), GTK_STATE_NORMAL, &color);
        else
            gtk_widget_override_background_color(GTK_WIDGET(file_browser->status_frame), GTK_STATE_NORMAL, NULL);
    }
    else
        gtk_widget_override_background_color(GTK_WIDGET(file_browser->status_frame), GTK_STATE_NORMAL, NULL);
#elif (GTK_MAJOR_VERSION == 2)
...
void on_status_bar_popup(GtkWidget* widget, GtkWidget* menu, PtkFileBrowser* file_browser)
{
    ...
    char* desc =
        g_strdup_printf("sep_bar1 status_border status_text panel%d_icon_status panel%d_font_status status_middle",
...
void ptk_file_browser_init(PtkFileBrowser* file_browser)
{
    ...
    if (xset_get_s_panel(file_browser->mypanel, "font_status"))
    {
        PangoFontDescription* font_desc =
            pango_font_description_from_string(xset_get_s_panel(file_browser->mypanel, "font_status"));
#if (GTK_MAJOR_VERSION == 3)
...
void ptk_file_browser_get_property(GObject* obj, uint prop_id, GValue* value, GParamSpec* pspec)
{
}

void ptk_file_browser_set_property(GObject* obj, uint prop_id, const GValue* value, GParamSpec* pspec)
{
}
...
void ptk_file_browser_update_views(GtkWidget* item, PtkFileBrowser* file_browser)
{
    ...
    {
        if ((evt_pnl_show->s || evt_pnl_show->ob2_data) && file_browser->side_toolbar && file_browser->side_toolbox &&
    ...
    {
        if ((evt_pnl_show->s || evt_pnl_show->ob2_data) &&
            (!file_browser->side_book_scroll || !gtk_widget_get_visible(file_browser->side_book_scroll)))
    ...
    if (xset_get_b_panel_mode(p, "show_book", mode) || xset_get_b_panel_mode(p, "show_dirtree", mode))
        gtk_widget_show(file_browser->side_vpane_bottom);
    else
        gtk_widget_hide(file_browser->side_vpane_bottom);

    if (xset_get_b_panel_mode(p, "show_devmon", mode) || xset_get_b_panel_mode(p, "show_dirtree", mode) ||
    ...
    gboolean large_icons = xset_get_b_panel(p, "list_icons") || xset_get_b_panel_mode(p, "list_large", mode);
...
{
    GtkWidget* label = gtk_notebook_get_tab_label(GTK_NOTEBOOK(file_browser->mynotebook), GTK_WIDGET(file_browser));
...
void ptk_file_browser_select_all(GtkWidget* item, PtkFileBrowser* file_browser)
{
    ...
    {
        GtkTreeSelection* tree_sel = gtk_tree_view_get_selection(GTK_TREE_VIEW(file_browser->folder_view));
...
void ptk_file_browser_select_pattern(GtkWidget* item, PtkFileBrowser* file_browser, const char* search_key)
{
    ...
    else
    {
        ...
        if (!xset_text_dialog(GTK_WIDGET(file_browser),
                              _("Select By Pattern"),
                              NULL,
                              FALSE,
                              _("Enter pattern to select files and folders:\n\nIf your pattern contains any uppercase "
                                "characters, the matching will be case sensitive.\n\nExample:  *sp*e?m*\n\nTIP: You "
                                "can also enter '%% PATTERN' in the path bar."),
                              NULL,
                              set->ob1,
                              &set->ob1,
                              NULL,
...
void on_folder_view_row_activated(GtkTreeView* tree_view, GtkTreePath* path, GtkTreeViewColumn* col,
                                  PtkFileBrowser* file_browser)
{
    // file_browser->button_press = FALSE;
    ptk_file_browser_open_selected_files(file_browser);
...
static void show_popup_menu(PtkFileBrowser* file_browser, GdkEventButton* event)
{
    ...
    popup = ptk_file_menu_new(file_browser, file_path, file, dir_name ? dir_name : cwd, sel_files);
    if (popup)
...
gboolean on_folder_view_button_release_event(GtkWidget* widget, GdkEventButton* event,
...
static GtkWidget* create_folder_view(PtkFileBrowser* file_browser, PtkFBViewMode view_mode)
{
    ...
    case PTK_FB_COMPACT_VIEW:
        ...
        else
        {
            ...
            exo_icon_view_set_item_width((ExoIconView*)folder_view, icon_size < 110 ? 110 : icon_size);
        }
        ...
        gtk_cell_layout_add_attribute(GTK_CELL_LAYOUT(folder_view),
                                      renderer,
                                      "pixbuf",
                                      file_browser->large_icons ? COL_FILE_BIG_ICON : COL_FILE_SMALL_ICON);
        ...
        exo_icon_view_enable_model_drag_source(EXO_ICON_VIEW(folder_view),
    ...
    case PTK_FB_LIST_VIEW:
        ...
        gtk_tree_view_enable_model_drag_source(GTK_TREE_VIEW(folder_view),
                                               (GDK_CONTROL_MASK | GDK_BUTTON1_MASK | GDK_BUTTON3_MASK),
                                               drag_targets,
                                               G_N_ELEMENTS(drag_targets),
                                               GDK_ACTION_ALL);
    ...
    {
        PangoFontDescription* font_desc =
            pango_font_description_from_string(xset_get_s_panel(file_browser->mypanel, "font_file"));
#if (GTK_MAJOR_VERSION == 3)
...
void init_list_view(PtkFileBrowser* file_browser, GtkTreeView* list_view)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(cols); i++)
    {
        ...
        else
        {
            ...
            XSet* set = xset_get_panel_mode(p, column_names[j], mode);
            int width = set->y ? atoi(set->y) : 100;
            if (width)
            {
                    ...
                    gtk_notebook_get_n_pages(GTK_NOTEBOOK(file_browser->mynotebook)) == 1)
                {
                    ...
                    PtkFileBrowser* first_fb =
                        (PtkFileBrowser*)gtk_notebook_get_nth_page(GTK_NOTEBOOK(file_browser->mynotebook), 0);
...
{
    return file_browser->file_list ? gtk_tree_model_iter_n_children(file_browser->file_list, NULL) : 0;
}
...
static char* folder_view_get_drop_dir(PtkFileBrowser* file_browser, int x, int y)
{
    ...
    if (tree_path)
    {
        ...
        if (file)
        {
            ...
            {
                dest_path =
                    g_build_filename(ptk_file_browser_get_cwd(file_browser), vfs_file_info_get_name(file), NULL);
                /*  this isn't needed?
...
void on_folder_view_drag_data_received(GtkWidget* widget, GdkDragContext* drag_context, int x, int y,
                                       GtkSelectionData* sel_data, uint info, uint time, gpointer user_data)
{
    char** list;
    char** puri;
        ...
        if (dest_dir)
        {
            ...
            if (file_browser->pending_drag_status)
            {
                ...
                if (stat(dest_dir, &statbuf) == 0)
                {
                    ...
                    if (file_browser->drag_source_dev != dest_dev || file_browser->drag_source_inode == dest_inode)
...
void on_folder_view_drag_data_get(GtkWidget* widget, GdkDragContext* drag_context, GtkSelectionData* sel_data,
                                  uint info, uint time, PtkFileBrowser* file_browser)
{
    ...
    for (sel = sels; sel; sel = g_list_next(sel))
    {
        ...
        char* full_path = g_build_filename(ptk_file_browser_get_cwd(file_browser), vfs_file_info_get_name(file), NULL);
...
gboolean on_folder_view_drag_motion(GtkWidget* widget, GdkDragContext* drag_context, int x, int y, uint time,
                                    PtkFileBrowser* file_browser)
{
    GtkTreeModel* model = NULL;
    GtkTreeIter it;
...
gboolean on_folder_view_drag_leave(GtkWidget* widget, GdkDragContext* drag_context, uint time,
                                   PtkFileBrowser* file_browser)
{
    /*  Don't call the default handler  */
...
gboolean on_folder_view_drag_drop(GtkWidget* widget, GdkDragContext* drag_context, int x, int y, uint time,
                                  PtkFileBrowser* file_browser)
{
...
{
    GList* sel_files = ptk_file_browser_get_selected_files(file_browser);
    ptk_open_files_with_app(ptk_file_browser_get_cwd(file_browser), sel_files, app_desktop, file_browser, FALSE, FALSE);
    vfs_file_info_list_free(sel_files);
}
...
void ptk_file_browser_copycmd(PtkFileBrowser* file_browser, GList* sel_files, char* cwd, char* setname)
{
    ...
    if ((g_str_has_prefix(setname, "copy_loc") || g_str_has_prefix(setname, "move_loc")) && !copy_dest && !move_dest)
    {
        char* folder;
        set2 = xset_get("copy_loc_last");
        if (set2->desc)
...
void ptk_file_browser_view_as_icons(PtkFileBrowser* file_browser)
{
    ...
    show_thumbnails(file_browser, PTK_FILE_LIST(file_browser->file_list), TRUE, file_browser->max_thumbnail);
...
void ptk_file_browser_on_action(PtkFileBrowser* browser, char* setname)
{
    ...
    else if (g_str_has_prefix(set->name, "status_"))
    {
        ...
        if (!strcmp(xname, "border") || !strcmp(xname, "text"))
            on_status_effect_change(NULL, browser);
        else if (!strcmp(xname, "name") || !strcmp(xname, "path") || !strcmp(xname, "info") || !strcmp(xname, "hide"))
