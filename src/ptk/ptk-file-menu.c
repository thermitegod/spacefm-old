...
//#define get_toplevel_win(data)  ( (GtkWindow*) (data->browser ? ( gtk_widget_get_toplevel((GtkWidget*) data->browser)
//) : NULL) )
...
static void on_popup_paste_link_activate(GtkMenuItem* menuitem,
                                         PtkFileMenu* data); // MOD added
static void on_popup_paste_target_activate(GtkMenuItem* menuitem,
                                           PtkFileMenu* data); // MOD added
static void on_popup_copy_text_activate(GtkMenuItem* menuitem,
                                        PtkFileMenu* data); // MOD added
static void on_popup_copy_name_activate(GtkMenuItem* menuitem,
                                        PtkFileMenu* data); // MOD added
void on_popup_copy_parent_activate(GtkMenuItem* menuitem,
                                   PtkFileMenu* data); // MOD added

void on_popup_paste_as_activate(GtkMenuItem* menuitem,
...
void ptk_file_menu_add_panel_view_menu(PtkFileBrowser* browser, GtkWidget* menu, GtkAccelGroup* accel_group)
{
    ...
    if (browser->view_mode == PTK_FB_LIST_VIEW)
    {
        ...
        desc = g_strdup_printf("panel%d_detcol_size panel%d_detcol_type panel%d_detcol_perm panel%d_detcol_owner "
                               "panel%d_detcol_date sep_v4 view_reorder_col",
                               p,
                               p,
                               p,
                               p,
    ...
    if (browser->file_list)
    {
        ...
        xset_set_ob2(set, NULL, NULL);
        set->b = PTK_FILE_LIST(browser->file_list)->sort_dir == PTK_LIST_SORT_DIR_FIRST ? XSET_B_TRUE : XSET_B_FALSE;
        set_radio = set;
        set = xset_set_cb("sortx_files", on_popup_sort_extra, browser);
        xset_set_ob2(set, NULL, set_radio);
        set->b = PTK_FILE_LIST(browser->file_list)->sort_dir == PTK_LIST_SORT_DIR_LAST ? XSET_B_TRUE : XSET_B_FALSE;
        set = xset_set_cb("sortx_mix", on_popup_sort_extra, browser);
        xset_set_ob2(set, NULL, set_radio);
        set->b = PTK_FILE_LIST(browser->file_list)->sort_dir == PTK_LIST_SORT_DIR_MIXED ? XSET_B_TRUE : XSET_B_FALSE;
    ...
    desc = g_strdup_printf("panel%d_list_detailed panel%d_list_compact panel%d_list_icons sep_v5 view_thumb "
                           "panel%d_list_large rubberband sep_v6 panel%d_font_file",
                           p,
                           p,
                           p,
                           p,
    ...
    desc = g_strdup_printf("panel%d_font_device panel%d_font_dir panel%d_font_book panel%d_font_files "
    ...
    desc = g_strdup_printf(
        "panel%d_show_toolbox panel%d_show_sidebar panel%d_show_devmon panel%d_show_book panel%d_show_dirtree sep_v7 "
        "panel%d_show_hidden view_list_style view_sortby view_columns sep_v8 view_refresh",
        p,
        p,
        p,
        p,
...
{ // either desktop or browser must be non-NULL
    ...
    if (browser)
    {
        ...
        static const char* copycmd[] = {
            "copy_loc",     "copy_loc_last", "copy_tab_prev",   "copy_tab_next",   "copy_tab_1",      "copy_tab_2",
            "copy_tab_3",   "copy_tab_4",    "copy_tab_5",      "copy_tab_6",      "copy_tab_7",      "copy_tab_8",
            "copy_tab_9",   "copy_tab_10",   "copy_panel_prev", "copy_panel_next", "copy_panel_1",    "copy_panel_2",
            "copy_panel_3", "copy_panel_4",  "move_loc",        "move_loc_last",   "move_tab_prev",   "move_tab_next",
            "move_tab_1",   "move_tab_2",    "move_tab_3",      "move_tab_4",      "move_tab_5",      "move_tab_6",
            "move_tab_7",   "move_tab_8",    "move_tab_9",      "move_tab_10",     "move_panel_prev", "move_panel_next",
            "move_panel_1", "move_panel_2",  "move_panel_3",    "move_panel_4"};
    ...
    if (browser)
    {
        ...
        static const char* permcmd[] = {
            "perm_r",           "perm_rw",         "perm_rwx",         "perm_r_r",          "perm_rw_r",
            "perm_rw_rw",       "perm_rwxr_x",     "perm_rwxrwx",      "perm_r_r_r",        "perm_rw_r_r",
            "perm_rw_rw_rw",    "perm_rwxr_r",     "perm_rwxr_xr_x",   "perm_rwxrwxrwx",    "perm_rwxrwxrwt",
            "perm_unstick",     "perm_stick",      "perm_go_w",        "perm_go_rwx",       "perm_ugo_w",
            "perm_ugo_rx",      "perm_ugo_rwx",    "rperm_rw",         "rperm_rwx",         "rperm_rw_r",
            "rperm_rw_rw",      "rperm_rwxr_x",    "rperm_rwxrwx",     "rperm_rw_r_r",      "rperm_rw_rw_rw",
            "rperm_rwxr_r",     "rperm_rwxr_xr_x", "rperm_rwxrwxrwx",  "rperm_rwxrwxrwt",   "rperm_unstick",
            "rperm_stick",      "rperm_go_w",      "rperm_go_rwx",     "rperm_ugo_w",       "rperm_ugo_rx",
            "rperm_ugo_rwx",    "own_myuser",      "own_myuser_users", "own_user1",         "own_user1_users",
            "own_user2",        "own_user2_users", "own_root",         "own_root_users",    "own_root_myuser",
            "own_root_user1",   "own_root_user2",  "rown_myuser",      "rown_myuser_users", "rown_user1",
            "rown_user1_users", "rown_user2",      "rown_user2_users", "rown_root",         "rown_root_users",
...
void app_job(GtkWidget* item, GtkWidget* app_item)
{
    ...
    case APP_JOB_REMOVE:
        ...
        if (strcmp(mime_type->type, "text/plain") && g_str_has_prefix(mime_type->type, "text/"))
            xset_msg_dialog(
                ...
                NULL,
                0,
                _("NOTE:  When compiling the list of applications to appear in the Open submenu for a text file, "
                  "SpaceFM will include applications associated with the MIME type (eg text/html) AND applications "
                  "associated with text/plain.  If you select Remove on an application, it will be removed as an "
                  "associated application for the MIME type (eg text/html), but will NOT be removed as an associated "
                  "application for text/plain (unless the MIME type is text/plain).  Thus using Remove may not remove "
                  "the application from the Open submenu for this type, unless you also remove it from text/plain."),
                NULL,
    ...
    case APP_JOB_EDIT:
        ...
        if (!g_file_test(path, G_FILE_TEST_EXISTS))
        {
            ...
            char* msg = g_strdup_printf(
                _("The file '%s' does not exist.\n\nBy copying '%s' to '%s' and editing it, you can adjust the "
                  "behavior and appearance of this application for the current user.\n\nCreate this copy now?"),
                path,
                share_desktop,
                path);
    ...
    case APP_JOB_ADD:
        path = ptk_choose_app_for_mime_type(
            data->browser ? GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(data->browser))) : GTK_WINDOW(data->browser),
            mime_type,
            FALSE,
            TRUE,
            TRUE,
    ...
    case APP_JOB_EDIT_TYPE:
        ...
        if (!g_file_test(path, G_FILE_TEST_EXISTS))
        {
            ...
            if (g_file_test(usr_path, G_FILE_TEST_EXISTS))
                msg = g_strdup_printf(
                    _("The file '%s' does not exist.\n\nBy copying '%s' to '%s' and editing it, you can adjust how "
                      "MIME type '%s' files are recognized for the current user.\n\nCreate this copy now?"),
                    path,
                    usr_path,
                    path,
                    mime_type->type);
                ...
                msg = g_strdup_printf(
                    _("The file '%s' does not exist.\n\nBy creating new file '%s' and editing it, you can define how "
            ...
            msg = g_strdup_printf(
                ...
                "file was generated by SpaceFM to allow you to change the name or icon\n     of the above mime type "
                "and to change the filename or magic patterns that\n     define this type.\n\n     IMPORTANT:  After "
                "saving this file, restart SpaceFM.  You may need to run:\n         update-mime-database "
                "~/.local/share/mime\n\n     Delete this file from ~/.local/share/mime/packages/ to revert to "
                ...
                "name/icon of PNG files (with optional translation):\n\n        <comment>Portable Network Graphics "
...
static void show_app_menu(GtkWidget* menu, GtkWidget* app_item, PtkFileMenu* data, uint button, guint32 time)
{
    ...
    // mime/packages/
    newitem =
        app_menu_additem(submenu, "mime/pac_kages/", GTK_STOCK_DIRECTORY, APP_JOB_BROWSE_MIME_USR, app_item, data);
    gtk_widget_set_sensitive(GTK_WIDGET(newitem),
                             !!data->browser && g_file_test("/usr/share/mime/packages", G_FILE_TEST_IS_DIR));
...
void on_new_bookmark(GtkMenuItem* menuitem, PtkFileMenu* data)
{
    ...
    {
        char* full_path =
            g_build_filename(data->cwd, vfs_file_info_get_name((VFSFileInfo*)data->sel_files->data), NULL);
...
void on_popup_paste_link_activate(GtkMenuItem* menuitem,
                                  PtkFileMenu* data) // MOD added
{
    if (data->browser)
...
void on_popup_paste_target_activate(GtkMenuItem* menuitem,
                                    PtkFileMenu* data) // MOD added
{
    if (data->browser)
...
void on_popup_copy_text_activate(GtkMenuItem* menuitem,
                                 PtkFileMenu* data) // MOD added
{
    ptk_clipboard_copy_as_text(data->cwd, data->sel_files);
}

void on_popup_copy_name_activate(GtkMenuItem* menuitem,
                                 PtkFileMenu* data) // MOD added
{
...
void on_popup_extract_list_activate(GtkMenuItem* menuitem, PtkFileMenu* data)
{
    /* If menuitem is set, function was called from GUI so files will contain
     * an archive */
    ptk_file_archiver_extract(data->browser, data->sel_files, data->cwd, NULL, HANDLER_LIST, menuitem ? TRUE : FALSE);
}
...
void ptk_file_menu_action(PtkFileBrowser* browser, char* setname)
{
    ...
    else if (g_str_has_prefix(set->name, "edit_"))
    {
        ...
        on_popup_copy_parent_activate(NULL, data);
             ...
             g_str_has_prefix(set->name, "copy_panel_") || g_str_has_prefix(set->name, "move_loc") ||
             g_str_has_prefix(set->name, "move_tab_") || g_str_has_prefix(set->name, "move_panel_"))
