...
{
    gtk_widget_set_sensitive(GTK_WIDGET(notebook), gtk_entry_get_text_length((GtkEntry*)widget) == 0);
    return FALSE;
}

void on_view_row_activated(GtkTreeView* tree_view, GtkTreePath* path, GtkTreeViewColumn* col, GtkWidget* dlg)
{
...
GtkWidget* app_chooser_dialog_new(GtkWindow* parent, VFSMimeType* mime_type, gboolean focus_all_apps,
...
static void on_dlg_response(GtkDialog* dlg, int id, gpointer user_data)
{
    ...
    case GTK_RESPONSE_DELETE_EVENT:
        ...
        if (task)
        {
            // g_printf("spacefm: app-chooser.c -> vfs_async_task_cancel\n");
...
void ptk_app_chooser_has_handler_warn(GtkWidget* parent, VFSMimeType* mime_type)
{
    ...
    GSList* handlers_slist =
        ptk_handler_file_has_handlers(HANDLER_MODE_FILE, HANDLER_MOUNT, NULL, mime_type, FALSE, FALSE, TRUE);
    if (handlers_slist)
    {
        msg = g_strdup_printf(
            _("Note:  MIME type '%s' is currently set to open with the '%s' file handler, rather than with your "
              "associated MIME application.\n\nYou may also need to disable this handler in Open|File Handlers for "
    ...
    else if (!xset_get_b("arc_def_open"))
    {
        ...
        handlers_slist =
            ptk_handler_file_has_handlers(HANDLER_MODE_ARC, HANDLER_EXTRACT, NULL, mime_type, FALSE, FALSE, TRUE);
        if (handlers_slist)
        {
            msg = g_strdup_printf(
                _("Note:  MIME type '%s' is currently set to open with the '%s' archive handler, rather than with your "
                  "associated MIME application.\n\nYou may also need to disable this handler in Open|Archive "
                  "Defaults|Archive Handlers, OR select global option Open|Archive Defaults|Open With App, for this "
