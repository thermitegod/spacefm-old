...
static void on_file_monitor_event(VFSFileMonitor* fm, VFSFileMonitorEvent event, const char* file_name,
                                  gpointer user_data);

static PtkDirTreeNode* ptk_dir_tree_node_new(PtkDirTree* tree, PtkDirTreeNode* parent, const char* path,
...
GType ptk_dir_tree_get_type(void)
{
    ...
    if (G_UNLIKELY(!type))
    {
        ...
        static const GInterfaceInfo tree_model_info = {(GInterfaceInitFunc)ptk_dir_tree_tree_model_init, NULL, NULL};

        static const GInterfaceInfo drag_src_info = {(GInterfaceInitFunc)ptk_dir_tree_drag_source_init, NULL, NULL};

        static const GInterfaceInfo drag_dest_info = {(GInterfaceInitFunc)ptk_dir_tree_drag_dest_init, NULL, NULL};
...
int ptk_dir_tree_node_compare(PtkDirTree* tree, PtkDirTreeNode* a, PtkDirTreeNode* b)
{
    ...
    ret = g_ascii_strcasecmp(vfs_file_info_get_disp_name(file2), vfs_file_info_get_disp_name(file1));
    return ret;
}

PtkDirTreeNode* ptk_dir_tree_node_new(PtkDirTree* tree, PtkDirTreeNode* parent, const char* path, const char* base_name)
{
...
void ptk_dir_tree_insert_child(PtkDirTree* tree, PtkDirTreeNode* parent, const char* file_path, const char* name)
{
    PtkDirTreeNode* node;
    ...
    case VFS_FILE_MONITOR_CREATE:
        if (G_LIKELY(!child))
        {
            ...
            if (node->n_children == 1 && !node->children->file)
                ...
                child = NULL;
            char* file_path = g_build_filename(fm->path, file_name, NULL);
            if (g_file_test(file_path, G_FILE_TEST_IS_DIR))
            {
