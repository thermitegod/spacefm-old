...
const char* handler_def_prefix[] = {"hand_arc_+", "hand_fs_+", "hand_net_+", "hand_f_+"};

// xset name prefixes of custom handlers
const char* handler_cust_prefix[] = {"hand_arc_", "hand_fs_", "hand_net_", "hand_f_"};

const char* handler_conf_xset[] = {"arc_conf2", "dev_fs_cnf", "dev_net_cnf", "open_hand"};

const char* dialog_titles[] = {N_("Archive Handlers"),
                               N_("Device Handlers"),
                               N_("Protocol Handlers"),
                               N_("File Handlers")};

const char* dialog_mnemonics[] = {N_("Archive Hand_lers"),
                                  N_("Device Hand_lers"),
...
const Handler handlers_net[] = {
    ...
    {"hand_net_+http",
     ...
     "missing_davfs=1 if you always want to open http in web browser\n# set missing_davfs=0 if you always want to "
     "mount http with davfs\nmissing_davfs=\n\nif [ -z \"$missing_davfs\" ];then\n    grep -qs "
     "'^[[:space:]]*allowed_types[[:space:]]*=[^#]*davfs' \\\n                                    "
     "/etc/udevil/udevil.conf 2>/dev/null\n    missing_davfs=$?  \nfi\nif [ \"$fm_url_proto\" = \"webdav\" ] || [ "
     ...
     "\"$fm_url_user$fm_url_password\" ]] && msg=\"\" || \\\n            msg=\"Warning: user:password in URL is not "
    ...
    {"hand_net_+ftp",
     ...
     "",
     "options=\"nonempty\"\nif [ -n \"%user%\" ];then\n    user=\",user=%user%\"\n    [[ -n \"%pass%\" ]] && "
     "user=\"$user:%pass%\"\nfi\n[[ -n \"%port%\" ]] && portcolon=:\necho \">>> curlftpfs -o $options$user "
     "ftp://%host%${portcolon}%port%%path% %a\"\necho\ncurlftpfs -o $options$user ftp://%host%${portcolon}%port%%path% "
    ...
    {"hand_net_+ssh",
     ...
     "\">>> sshfs -p $fm_url_port $fm_url_user$fm_url_host:$fm_url_path %a\"\necho\n# Run sshfs through nohup to "
     "prevent disconnect on terminal close\nsshtmp=\"$(mktemp --tmpdir spacefm-ssh-output-XXXXXXXX.tmp)\" || exit "
     ...
     "Method - if enabled, disable nohup line above and\n#                    uncheck Run In Terminal\n# # Run sshfs "
     "in a terminal without SpaceFM task.  sshfs disconnects when the\n# # terminal is closed\n# spacefm -s run-task "
     "cmd --terminal \"echo 'Connecting to $fm_url'; echo; sshfs -p $fm_url_port $fm_url_user$fm_url_host:$fm_url_path "
     "%a; if [ $? -ne 0 ];then echo;read -p '[ Finished ] Press Enter to close'; else echo;read -p 'Press Enter to "
     "close (closing this window may unmount sshfs)';fi\" & sleep 1\n",
     TRUE,
     "fusermount -u \"%a\"",
     FALSE,
     INFO_EXAMPLE,
    ...
    {"hand_net_+mtp",
     ...
     "go-mtpfs in background, as it does not exit after mount\n    outputtmp=\"$(mktemp --tmpdir "
     "spacefm-go-mtpfs-output-XXXXXXXX)\" || exit 1\n    go-mtpfs \"%a\" &> \"$outputtmp\" &\n    sleep 2s\n    [[ -e "
     "\"$outputtmp\" ]] && cat \"$outputtmp\" ; rm -f \"$outputtmp\"\n    # set success status only if positive that "
    ...
    {"hand_net_+udevilsmb",
     ...
     "smb mtab_fs=cifs",
     "",
     "UDEVIL_RESULT=\"$(udevil mount \"$fm_url\" | grep Mounted)\"\n[ -n \"$UDEVIL_RESULT\" ] && spacefm -s set "
     "new_tab \"${UDEVIL_RESULT#* at }\"",
     TRUE,
     "udevil umount \"%a\"",
     FALSE,
     INFO_EXAMPLE,
...
const Handler handlers_file[] = {
    ...
    {"hand_f_+iso",
     ...
     "-z \"$fuse\" ]];then\n    udevil=\"$(which udevil)\"\n    if [[ -z \"$udevil\" ]];then\n        echo \"You must "
     "install fuseiso or udevil to mount ISOs with this handler.\"\n        exit 1\n    fi\n    # use udevil - attempt "
...
{ /* if handler->disable, get const command, else get script path if exists */
    ...
    // name script
    char* str =
        g_strdup_printf("/hand-%s-%s.sh", modes[mode], mode == HANDLER_MODE_ARC ? cmds_arc[cmd] : cmds_mnt[cmd]);
...
GSList* ptk_handler_file_has_handlers(int mode, int cmd, const char* path, VFSMimeType* mime_type, gboolean test_cmd,
                                      gboolean multiple, gboolean enabled_only)
{ /* this function must be FAST - is run multiple times on menu popup
...
void ptk_handler_import(int mode, GtkWidget* handler_dlg, XSet* set)
{
    ...
    char* dis_name =
        g_strdup_printf("%s %s", new_handler_xset->menu_label, new_handler_xset->b == XSET_B_TRUE ? "" : disabled);
...
static void config_load_handler_settings(XSet* handler_xset, char* handler_xset_name, const Handler* handler,
                                         HandlerData* hnd)
    ...
    for (i = 0; archive_handlers[i] != NULL; ++i)
    {
        if (g_str_has_prefix(archive_handlers[i], handler_cust_prefix[hnd->mode]))
        {
            ...
            if (handler_xset)
            {
                ...
                const char* disabled = hnd->mode == HANDLER_MODE_FILE ? _("(optional)") : _("(disabled)");
                char* dis_name =
                    g_strdup_printf("%s %s", handler_xset->menu_label, handler_xset->b == XSET_B_TRUE ? "" : disabled);
                gtk_list_store_set(GTK_LIST_STORE(hnd->list),
                                   &iter,
                                   COL_XSET_NAME,
                                   archive_handlers[i],
...
static void on_configure_button_press(GtkButton* widget, HandlerData* hnd)
{
    ...
    if (GTK_WIDGET(widget) == GTK_WIDGET(hnd->btn_add))
    {
        ...
        new_handler_xset->b =
            gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(hnd->chkbtn_handler_enabled)) ? XSET_B_TRUE : XSET_B_FALSE;
        new_handler_xset->disable = FALSE; // not default - save in session
        ...
        // Adding handler to model
        const char* disabled = hnd->mode == HANDLER_MODE_FILE ? _("(optional)") : _("(disabled)");
        char* dis_name = g_strdup_printf("%s %s", handler_name, new_handler_xset->b == XSET_B_TRUE ? "" : disabled);
        gtk_list_store_set(GTK_LIST_STORE(hnd->list),
                           &iter,
                           COL_XSET_NAME,
                           new_handler_xset->name,
        ...
        else
        {
            // Adding new handler to handlers
            char* new_handlers_list =
                g_strdup_printf("%s %s", new_handler_xset->name, xset_get_s(handler_conf_xset[hnd->mode]));
    ...
    else if (GTK_WIDGET(widget) == GTK_WIDGET(hnd->btn_up) || GTK_WIDGET(widget) == GTK_WIDGET(hnd->btn_down))
    {
        if (!handler_xset)
            // no row selected
            goto _clean_exit;
        ...
        do
        {
            ...
            if (iter.stamp == it.stamp && iter.user_data == it.user_data && iter.user_data2 == it.user_data2 &&
...
static gboolean validate_archive_handler(HandlerData* hnd)
{
    ...
    if (g_strcmp0(handler_compress, "") != 0)
    {
         ...
         * type */
        if ((!g_strstr_len(handler_compress, -1, "%o") && !g_strstr_len(handler_compress, -1, "%O")) ||
            (!g_strstr_len(handler_compress, -1, "%n") && !g_strstr_len(handler_compress, -1, "%N")))
        {
            xset_msg_dialog(GTK_WIDGET(hnd->dlg),
                            GTK_MESSAGE_WARNING,
                            _(dialog_titles[hnd->mode]),
...
gboolean on_activate_link(GtkLabel* label, char* uri, HandlerData* hnd)
{
...
{ // also used on dlg keypress
    ...
    {
        int keymod = (event->state & (GDK_SHIFT_MASK | GDK_CONTROL_MASK | GDK_MOD1_MASK | GDK_SUPER_MASK |
...
void on_entry_text_insert(GtkEntryBuffer* buffer, uint position, char* chars, uint n_chars, HandlerData* hnd)
{
    if (!hnd->changed)
    {
        hnd->changed = TRUE;
...
void on_entry_text_delete(GtkEntryBuffer* buffer, uint position, uint n_chars, HandlerData* hnd)
{
...
static GtkWidget* add_popup_menuitem(GtkWidget* popup, GtkAccelGroup* accel_group, const char* label, const char* image,
                                     int job, HandlerData* hnd)
{
#if (GTK_MAJOR_VERSION == 3)
...
static void on_options_button_clicked(GtkWidget* btn, HandlerData* hnd)
{
    ...
    item = add_popup_menuitem(popup, accel_group, _("_Export"), GTK_STOCK_SAVE, HANDLER_JOB_EXPORT, hnd);
    gtk_widget_set_sensitive(item, handler_selected);

    add_popup_menuitem(popup, accel_group, _("Import _File"), GTK_STOCK_ADD, HANDLER_JOB_IMPORT_FILE, hnd);
    add_popup_menuitem(popup, accel_group, _("Import _URL"), GTK_STOCK_NETWORK, HANDLER_JOB_IMPORT_URL, hnd);
    ...
    if (btn)
    {
        ...
        if (hnd->mode == HANDLER_MODE_ARC)
        {
            ...
            set->desc =
                g_strdup("arc_def_open arc_def_ex arc_def_exto arc_def_list sep_arc1 arc_def_parent arc_def_write");
...
void ptk_handler_show_config(int mode, PtkFileBrowser* file_browser, XSet* def_handler_set)
{
    ...
    gtk_label_set_markup_with_mnemonic(GTK_LABEL(lbl_handler_mime),
                                       mode == HANDLER_MODE_ARC || mode == HANDLER_MODE_FILE ? _("MIM_E Type:")
                                                                                             : _("Whit_elist:"));
    ...
    gtk_label_set_markup_with_mnemonic(GTK_LABEL(lbl_handler_extension),
                                       mode == HANDLER_MODE_ARC || mode == HANDLER_MODE_FILE ? _("P_athname:")
                                                                                             : _("Bl_acklist:"));
    ...
    gtk_label_set_markup_with_mnemonic(GTK_LABEL(lbl_handler_extract),
                                       mode == HANDLER_MODE_ARC ? _("<b>Ex_tract:</b>") : _("<b>Unmoun_t:</b>"));
    ...
    gtk_label_set_markup_with_mnemonic(GTK_LABEL(lbl_handler_list),
                                       mode == HANDLER_MODE_ARC ? _("<b>Li_st:</b>") : _("<b>Propertie_s:</b>"));
    ...
    if (hnd->entry_handler_icon)
    {
        ...
        g_signal_connect(G_OBJECT(gtk_entry_get_buffer(GTK_ENTRY(hnd->entry_handler_icon))),
                         ...
                         G_CALLBACK(on_entry_text_delete),
                         hnd);
        g_signal_connect(G_OBJECT(hnd->icon_choose_btn), "clicked", G_CALLBACK(on_icon_choose_button_clicked), hnd);
    }
