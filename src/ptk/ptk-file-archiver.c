...
static void on_format_changed(GtkComboBox* combo, gpointer user_data)
{
    ...
    if (handler_xset)
    {
        ...
        char* err_msg =
            ptk_handler_load_script(HANDLER_MODE_ARC, HANDLER_COMPRESS, handler_xset, GTK_TEXT_VIEW(view), NULL);
        if (err_msg)
        {
...
void ptk_file_archiver_create(PtkFileBrowser* file_browser, GList* files, const char* cwd)
{
    ...
    gtk_widget_set_focus_on_click(GTK_BUTTON(gtk_dialog_add_button(GTK_DIALOG(dlg), GTK_STOCK_HELP, GTK_RESPONSE_HELP)),
                                  FALSE);
#elif (GTK_MAJOR_VERSION == 2)
    gtk_button_set_focus_on_click(GTK_BUTTON(gtk_dialog_add_button(GTK_DIALOG(dlg), GTK_STOCK_HELP, GTK_RESPONSE_HELP)),
                                  FALSE);
#endif
    ...
    while ((res = gtk_dialog_run(GTK_DIALOG(dlg))))
    {
        if (res == GTK_RESPONSE_OK)
        {
            ...
            char* err_msg =
                ptk_handler_load_script(HANDLER_MODE_ARC, HANDLER_COMPRESS, handler_xset, NULL, &compress_cmd);
            if (err_msg)
            {
                g_warning("%s", err_msg);
                g_free(err_msg);
            ...
            if (g_strcmp0(compress_cmd, command))
            {
                ...
                if (handler_xset->disable)
                {
                    ...
                    err_msg =
                        ptk_handler_save_script(HANDLER_MODE_ARC, HANDLER_EXTRACT, handler_xset, NULL, compress_cmd);
                    if (err_msg)
                    {
                        g_warning("%s", err_msg);
                        g_free(err_msg);
    ...
    if (g_strstr_len(command, -1, "%O"))
    {
        ...
        for (i = 0, l = files; l && (i == 0 || g_strstr_len(command, -1, "%N")); l = l->next, ++i)
        {
            ...
            else
            {
                s1 = final_command;
                final_command =
                    g_strconcat(final_command, "echo\n", cmd_to_run, "\n[[ $? -eq 0 ]] || fm_handle_err\n", NULL);
                g_free(s1);
            }
            g_free(cmd_to_run);
        }
...
{ /* This function is also used to list the contents of archives */
    ...
    if (!dest_dir && !list_contents)
    {
        ...
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(chk_write), xset_get_int("arc_dlg", "z") == 1 && geteuid() != 0);
        ...
        while ((res = gtk_dialog_run(GTK_DIALOG(dlg))))
        {
            if (res == GTK_RESPONSE_OK)
            {
                ...
                write_access = create_parent && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(chk_write));
    ...
    for (l = files; l; l = l->next)
    {
        ...
        handlers_slist =
            ptk_handler_file_has_handlers(HANDLER_MODE_ARC, archive_operation, full_path, mime_type, TRUE, FALSE, TRUE);
        if (handlers_slist)
        {
