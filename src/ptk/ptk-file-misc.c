...
void ptk_delete_files(GtkWindow* parent_win, const char* cwd, GList* sel_files, GtkTreeView* task_view)
{
    ...
    if (!app_settings.no_confirm)
    {
        ...
        GtkWidget* dlg = gtk_message_dialog_new(parent_win, GTK_DIALOG_MODAL, GTK_MESSAGE_WARNING, GTK_BUTTONS_YES_NO, msg, NULL);
...
void on_move_change(GtkWidget* widget, MoveSet* mset)
{
    ...
    if (mset->create_new)
    {
        ...
        if (new_folder ||
            (new_link && g_file_test(gtk_entry_get_text(GTK_ENTRY(mset->entry_target)), G_FILE_TEST_IS_DIR) &&
    ...
    */
        ...
        mset->path_exists_file != path_exists_file || mset->mode_change)
    {
        ...
        if (full_path_same && (mset->create_new == PTK_RENAME || mset->create_new == PTK_RENAME_NEW_LINK))
        {
    ...
    if (mset->create_new && gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(mset->opt_new_link)))
    {
        ...
        gtk_widget_set_sensitive(
            mset->next,
            (path && path[0] != '\0' && !(full_path_same && full_path_exists) && !full_path_exists_dir));
        g_free(path);
    }
...
void on_browse_mode_toggled(GtkMenuItem* item, GtkWidget* dlg)
{
    ...
    for (i = MODE_FILENAME; i <= MODE_PATH; i++)
    {
        ...
        {
            int action = i == MODE_PARENT ? GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER : GTK_FILE_CHOOSER_ACTION_SAVE;
            GtkAllocation allocation;
            gtk_widget_get_allocation(GTK_WIDGET(dlg), &allocation);
...
void on_browse_button_press(GtkWidget* widget, MoveSet* mset)
{
    ...
    GtkWidget* dlg = gtk_file_chooser_dialog_new(_("Browse"),
                                                 mset->parent ? GTK_WINDOW(mset->parent) : NULL,
                                                 mode_default == MODE_PARENT ? GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER
    ...
    mode[MODE_PARENT] =
        gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(mode[MODE_FILENAME]), _("Pa_rent"));
...
void on_opt_toggled(GtkMenuItem* item, MoveSet* mset)
{
    ...
    if (as_root)
        ...
        win_icon = "gtk-edit";
    GdkPixbuf* pixbuf =
        gtk_icon_theme_load_icon(gtk_icon_theme_get_default(), win_icon, 16, GTK_ICON_LOOKUP_USE_BUILTIN, NULL);
    if (pixbuf)
    ...
    gtk_button_set_image(GTK_BUTTON(mset->next),
                         xset_get_image(as_root ? "GTK_STOCK_DIALOG_WARNING" : "GTK_STOCK_YES", GTK_ICON_SIZE_BUTTON));
...
void on_toggled(GtkMenuItem* item, MoveSet* mset)
{
    ...
    if (!gtk_widget_get_visible(mset->opt_copy) && !gtk_widget_get_visible(mset->opt_link) &&
        !gtk_widget_get_visible(mset->opt_copy_target) && !gtk_widget_get_visible(mset->opt_link_target))
    {
...
int ptk_rename_file(PtkFileBrowser* file_browser, const char* file_dir, VFSFileInfo* file, const char* dest_dir,
                    gboolean clip_copy, PtkRenameMode create_new, AutoOpenCreate* auto_open)
{
    if (!file_dir)
        return 0;
    ...
    */
    ...
    mset->input_full_path =
        GTK_WIDGET(multi_input_new(GTK_SCROLLED_WINDOW(mset->scroll_full_path), mset->new_path, FALSE));
    ...
    mset->opt_copy_target =
        gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(mset->opt_move), _("Copy _Target"));
    mset->opt_link_target =
        gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(mset->opt_move), _("Link Tar_get"));
    ...
    mset->opt_new_folder = gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(mset->opt_new_file),
                                                                          C_("New|Radio", "Fol_der"));
    mset->opt_new_link =
        gtk_radio_button_new_with_mnemonic_from_widget(GTK_RADIO_BUTTON(mset->opt_new_file), C_("New|Radio", "_Link"));
    ...
    while ((response = gtk_dialog_run(GTK_DIALOG(mset->dlg))))
    {
        if (response == GTK_RESPONSE_OK || response == GTK_RESPONSE_APPLY)
        {
            ...
            else if (create_new && new_file)
            {
                ...
                if (!from_path)
                    ...
                    task->task->exec_command =
                        g_strdup_printf("%s%scp -f %s %s", root_mkdir, over_cmd, from_path, to_path);
                g_free(from_path);
                g_free(to_path);
                g_free(over_cmd);
                task->task->exec_sync = TRUE;
            ...
            else if (copy || copy_target)
            {
                ...
                {
                    task->task->exec_command =
                        g_strdup_printf("%scp -Pf%s %s %s", root_mkdir, over_opt, from_path, to_path);
                }
                g_free(from_path);
                g_free(to_path);
                g_free(over_opt);
...
gboolean ptk_create_new_file(GtkWindow* parent_win, const char* cwd, gboolean create_folder, VFSFileInfo** file)
{
    ...
    while (gtk_dialog_run(GTK_DIALOG(dlg)) == GTK_RESPONSE_OK)
    {
        looponce = TRUE; // MOD
        int result;
        char* file_name = ptk_input_dialog_get_text(dlg);
        char* full_path = g_build_filename(cwd, file_name, NULL);
        g_free(file_name);
        if (g_file_test(full_path, G_FILE_TEST_EXISTS))
        {
            GtkLabel* prompt = GTK_LABEL(ptk_input_dialog_get_label(dlg));
                                           ...
                                           VFSMimeType* mime_type)
{
    ...
    GSList* handlers_slist =
        ptk_handler_file_has_handlers(HANDLER_MODE_ARC, cmd, full_path, mime_type, FALSE, FALSE, TRUE);
    if (handlers_slist)
    {
...
static void open_files_with_handler(ParentInfo* parent, GList* files, XSet* handler_set)
{
    ...
    gboolean multiple = (strstr(command, "%N") || strstr(command, "%F") || strstr(command, "fm_files[") ||
...
static gboolean open_files_with_app(ParentInfo* parent, GList* files, const char* app_desktop)
{
    ...
    if (app_desktop && g_str_has_prefix(app_desktop, "###") && (handler_set = xset_is(app_desktop + 3)) && files)
    {
        // is a handler
    ...
    else if (app_desktop)
    {
        ...
        {
            GtkWidget* toplevel =
                parent->file_browser ? gtk_widget_get_toplevel(GTK_WIDGET(parent->file_browser)) : NULL;
...
void ptk_open_files_with_app(const char* cwd, GList* sel_files, const char* app_desktop, PtkFileBrowser* file_browser,
    ...
    for (l = sel_files; l; l = l->next)
    {
        ...
        if (G_LIKELY(full_path))
        {
            ...
            else
            {
                ...
                if (!xnever && vfs_file_info_is_executable(file, full_path) && (!app_settings.no_execute || xforce))
                {
                    char* argv[2] = {full_path, NULL};
                    GdkScreen* screen =
                        file_browser ? gtk_widget_get_screen(GTK_WIDGET(file_browser)) : gdk_screen_get_default();
                    GError* err = NULL;
                ...
                if (!alloc_desktop && vfs_file_info_is_symlink(file))
                {
                    ...
                    if (target_path)
                    {
                        ...
                        {
                            char* msg = g_strdup_printf(_("This symlink's target is missing or you do not have "
                ...
                if (!alloc_desktop)
                {
                    ...
                    toplevel = file_browser ? gtk_widget_get_toplevel(GTK_WIDGET(file_browser)) : NULL;
                    alloc_desktop =
                        ptk_choose_app_for_mime_type((GtkWindow*)toplevel, mime_type, TRUE, TRUE, TRUE, !file_browser);
                }
                if (!alloc_desktop)
                {
                    g_free(full_path);
    ...
    if (missing_targets > 0)
    {
        ...
        ptk_show_error(parent,
                       ...
                       g_strdup_printf("%i target%s missing",
                                       missing_targets,
                                       missing_targets > 1 ? g_strdup_printf("s are") : g_strdup_printf(" is")));
    }
}
...
void ptk_file_misc_rootcmd(PtkFileBrowser* file_browser, GList* sel_files, char* cwd, char* setname)
{
    ...
    {
        char* file_path = g_build_filename(cwd, vfs_file_info_get_name((VFSFileInfo*)sel->data), NULL);
        char* file_path_q = bash_quote(file_path);
        str = file_paths;
    ...
    if (!strcmp(setname, "root_delete"))
    {
        ...
        {
            str = g_strdup_printf(
                ngettext("Delete %d selected item as root ?", "Delete %d selected items as root ?", item_count),
                item_count);
    ...
    else
    {
        ...
        if (set->desc)
            ...
            folder = cwd;
        char* path = xset_file_dialog(parent, GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, _("Choose Location"), folder, NULL);
